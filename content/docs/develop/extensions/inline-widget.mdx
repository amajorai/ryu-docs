...
(`plugins-store/sample-widget/`) and adapt it. It is a zero-dependency Node MCP server —
...
(e.g. the plugin lacks the `widget:render` grant). Return `isError: false` (the default) — **an error
