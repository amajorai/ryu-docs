...
## Finish onboarding with a real task

At the end of first-run setup, Ryu can suggest apps based on the work context
you connect, then draft a first task for the agent. The activation steps include
a visual comparison of hiring overhead and Ryu's fixed offer, plus connection
quests worth **$0.50 each** in Ryu Fast bonus credits, up to **$10**.

The desktop offer is **$50 for the first month**, followed by **$250/month from
month two onward** at the five-seat Teams floor. An existing subscriber skips
checkout. Otherwise, Ryu confirms the subscription before creating the task;
canceling checkout never starts it. Ryu lives where you already work, so the
first task can use the apps you already connected without moving your workflow.

Profile-derived recommendations and first-task creation are available only to
the owner or an admin of an eligible personal/self-hosted or managed workspace.
Members using a shared node see the workspace-owner path instead.
