...
- **Compute** - choose a bounded CPU or GPU workcell from the exchange.
- **Share** - configure how an idle node or approved agent could be offered.
...
At the bottom of **Settings → General**, **Open source licenses** opens a read-only list of the third-party notices for dependencies that may be bundled with Ryu Desktop. The list includes the JavaScript packages and native Rust crates used by the desktop build, along with the license or notice text published by each dependency when available.

## General preferences

**Settings → General** keeps the Desktop defaults that shape a new workspace:

- **Projectless task folder** chooses the local working directory for a new chat that does not have an explicit project. Existing conversations and selected projects keep their own folders.
- **Default file open destination** chooses the OS file manager or an installed editor for the workspace file tree's Open action.
- **Language** defaults to **Auto detect** and can be fixed to English or an installed language pack. Changes apply immediately.
- **Show in menu bar** keeps Ryu available from the macOS menu bar (or the system tray on other desktop platforms) when the main window is closed.
- **Bottom panel** controls whether the bottom-panel button appears in the chat header.
- **Default terminal location** chooses whether terminal shortcuts and project environment actions open the terminal in the bottom or right workspace dock.
- **Prevent sleep while running** keeps the local computer awake while an ACP agent is active; the display may still turn off.

These preferences are local to the Desktop client except for sleep prevention, which is also reflected in the active node's Gateway ACP policy. The native sleep inhibitor is active only while a local ACP agent is running.
...
When the sidebar count increases, the notification bell gives a short ring and the
count rolls to its new value. The bell stays quiet when notifications are read or
archived, and the animation follows the system's reduced-motion preference.
...
Its scheduled,
live and expired indicators pause while the page is hidden and catch up when you
return, without changing the announcement schedule.
...
Ryu Work also starts in the built-in **Agents view** sidebar. Direct threads appear below
their bot, while group and otherwise-unassigned conversations stay in **Other chats** below
the roster. The view has no separate Sessions toggle, and it is built into Desktop — no
separate app or plugin is required. Code users can choose another sidebar arrangement from
the same Appearance controls or the sidebar context menu.
...
- Drag a tab onto a pane edge to preview and create a split there. Dropping in the
  center of an existing split swaps the two panes.
...
In Desktop, drag a tab from the title bar or vertical tab list beyond the edge of
its window and release. Ryu moves that tab into a separate window, with the sidebar
closed and the other tabs left in the original window. A tab in a group or split
moves on its own; the remaining group or panes stay together.

You can also right-click a tab and choose **Move tab to new window**. **Open in new
window** keeps the original tab open as a separate copy. Moving waits for the new
window to become ready; if it cannot open, your source tab stays in place. Dropping
inside the original window keeps the existing reorder and split gestures.

The moved tab keeps its selected node, unsent chat text and attachments, model and
agent selections, workspace dock state, and workspace artifact content. Save a
temporary chat with existing messages before moving it, since its history belongs
to its current window. Native tab tear-out is available in Desktop.
...
Desktop unloads inactive tab trees after **10 minutes** by default to keep long-lived workspaces
responsive. Change this in **Settings → General → Tabs → Unload inactive tabs**; **Never** keeps
every tab mounted. This does not delete a conversation or its transcript: it only releases the
inactive route's in-memory state, and focusing the tab loads the durable transcript again. Active,
pinned, split-visible, and running tabs are protected.

In the normal title-bar strip, double-click a colored tab-group pill to rename the
group inline. Double-click a saved pane-layout preset's name in **Settings → General
→ Tabs** to edit that name inline. Press **Enter** to commit either rename or
**Esc** to cancel.
...
Double-click a tab's title to rename it inline. In the normal title-bar strip, the
same gesture also renames a colored tab-group pill. The rename applies where the tab
is named for something you own:
...
On macOS, Ryu uses the system's semantic sidebar vibrancy under the webview. Other desktop platforms use their available compositor and CSS backdrop support. Both controls are off by default, apply immediately, and are included in Appearance reset and settings sync.
