...
**Passport** lets an agent use a website connection on your behalf without placing credential state
in the model. You store a connection for a domain once, attach the profile to an agent, and when a
tool call targets that domain Core consults the Gateway before Passport consumes any credential.

Credentials are:

- **Encrypted at rest** in Passport's tenant store (`enc:v1:` envelope).
...
<Callout type="info">
  Passport consumes credentials only through an explicit governed runtime path. Core's built-in
  `web_fetch` and authenticated MCP paths use the Passport adapter when remote mode is configured;
  other tools still need their own declared credential consumer. Connect owns Composio account and
...
<Callout type="warn">
  Manual import is the working capture path for provider-neutral Passport connections. Composio
  account linking and actions belong to the separate [Connect](/docs/standalone/connect) service;
  Passport does not silently capture or export their provider credentials.
</Callout>
...
Existing Desktop clients call these Core adapter routes (port 7980). New standalone integrations
should use the [Passport API](/docs/standalone/passport-api) directly.
...
Continue with [Desktop user guide](/docs/surfaces/desktop/user-guide), [Passport](/docs/standalone/passport), [Gateway governance](/docs/gateway/governance), [authentication and pairing](/docs/security/authentication-and-pairing), and [Desktop account security](/docs/surfaces/desktop/user-guide/account-security).
