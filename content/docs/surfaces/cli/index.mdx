...
The terminal clients keep the Core contract while adapting interaction to a keyboard and a small
viewport:

| Capability | Terminal behavior |
|---|---|
| Core transport | Stream chat and lists over HTTP and Server-Sent Events (SSE); the terminal does not run inference |
| Keyboard navigation | Use a fuzzy command palette instead of a large sidebar or mouse-first navigation |
| Data views | Render portable list, table, and data-table contributions; unsupported rich views report unavailable |
| Native effects | Use Core notifications and status rows; do not assume browser permissions, haptics, Live Activities, or Dynamic Island |

The OpenTUI client is documented in [OpenTUI terminal client](/docs/surfaces/cli/tui). The
[desktop parity matrix](/docs/surfaces/cli/parity) explains which rich Desktop features stay out of
...
| `/group <id>` | Route the turn to an agent group instead of a single agent (sent as `team_id`). |
...
<Cards>
  ...
  <Card title="Desktop" description="The primary surface the CLI mirrors, also over the same Core" href="/docs/surfaces/desktop" />
</Cards>
