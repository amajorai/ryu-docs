...
terminal clients are additional surfaces that can connect to the same Core node. Ryu Infra can be
Ryu Cloud managed or self-hosted. When a client connects to the same node, a chat keeps its
...
<Cards>
  ...
  <DocCard href="/docs/surfaces/browser-extension" />
...
|---|---|
| Ryu OS / desktop workspace | Dock, live App windows, Mission Control, and desktop wallpaper controls |
| Ryu Console / desktop app | Native windows, local engines and sidecars, and media picture-in-picture (PiP) |
| Mobile app | `llama.rn` on-device models, notifications, Live Activities, haptics, and device tools |
| Browser app | Anonymous browser sessions, local Core setup, Cloud fallback, and browser navigation |
| Browser extension | Browser-local Transformers.js inference, in-page copilot, and browser tools |
