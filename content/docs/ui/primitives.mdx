...
Choose the narrowest layer that owns the problem, then let the shared primitives carry the visual
contract through the product surface:

```mermaid
flowchart LR
  Surface["Surface app\nstate + permissions"] --> Blocks["@ryu/blocks\nproduct composition"]
  Blocks --> UI["@ryu/ui\ncontrols + agent UI"]
  UI --> Theme["Theme tokens\nsemantic colors + motion"]
```
...
`TabsList` includes a sliding active indicator for the default, segmented, pill,
muted-pill, and line variants. Selection changes use a 250 ms transition with
`cubic-bezier(0.22, 1, 0.36, 1)` easing. The indicator snaps into place on first
paint and layout changes, and reduced-motion preferences disable the slide.
Text and stepper variants retain their label and step treatments. To customize
the marker, place one `TabsIndicator` inside the list; it replaces the automatic
