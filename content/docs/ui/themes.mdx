---
title: "Themes"
description: "Apply semantic token presets, contrast curves, typography, and custom themes at runtime."
---

The theme system applies CSS custom properties to the document root. Components consume semantic
tokens, so a surface can change its visual treatment without changing component code.

## Modules

| Module | Import | Responsibility |
|---|---|---|
| Presets | `@ryu/ui/theme/presets` | Theme variants and token maps |
| Apply | `@ryu/ui/theme/apply` | Write tokens, contrast, radius, and fonts to the DOM |
| Preferences | `@ryu/ui/theme/prefs` | Persist mode, preset, typography, and custom theme preferences |

## Preset IDs

The package includes light and dark preset IDs for Ryu, editor-oriented palettes, and several
well-known color systems. Use the exact IDs exported by `presets.ts` when storing a preference.

```tsx
import { applyVariant, applyRadius, applyFonts } from "@ryu/ui/theme/apply";
import { findVariantIn } from "@ryu/ui/theme/presets";

const theme = findVariantIn("ryu-dark");
if (theme) applyVariant(theme);
applyRadius(0.625);
applyFonts("Inter", "Geist", "JetBrains Mono");
```

## Custom themes

Convert a preset to editable tokens, change the values, and turn it back into a `ThemeVariant`:

```tsx
import {
  customTokensToVariant,
  variantToCustomTokens,
} from "@ryu/ui/theme/presets";

const tokens = variantToCustomTokens(myVariant);
tokens["--primary"] = "oklch(0.5 0.2 250)";

const custom = customTokensToVariant("my-theme", "My Theme", "dark", tokens);
```

## Reduced motion

All shared animations respect `prefers-reduced-motion: reduce`. Keep that behavior when adding
surface-specific transitions.

Continue with [Ryu UI](/docs/ui), [UI getting started](/docs/ui/getting-started), [UI primitives](/docs/ui/primitives), [UI components](/docs/ui/components), and [UI hooks](/docs/ui/hooks).

## Status labels

Use `text-status-success`, `text-status-warning`, `text-status-info`, and
`text-status-destructive` for small status text. These derive readable text tones
from the active status color and foreground. Keep base status colors for fills
and indicators. The lightweight `@ryu/ui/status-colors.css` import provides the
same text roles to hosts that supply their own token and stylesheet adapter.
