...
The URL can be a private Tailscale or Headscale address. For Tailcat, use a Tailcat client/proxy that exposes
the temporary connection to the MCP host's HTTP client; the Tailcat address itself is a connection token, not a
URL. The network layer handles reachability, while Core still checks the bearer on every request.
...
For credentials, keep the value out of `mcp.json` and store it in the
[Secret Vault](/docs/security/secret-vault). Use a `secret:NAME` reference in a server's
`headers` or `env` map; Core resolves it only during the governed MCP call. The reference
does not grant the server access by itself: registration, the agent allowlist, node scope,
Gateway policy, and any approval still apply.
