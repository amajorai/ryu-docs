...
See [MCP Server](/docs/extend/mcp) for the full setup and configuration.
...
flowchart TD
  subgraph Sources["Tool Sources"]
    ...
    BUILTIN["Built-in providers<br/>(Core-synthesized tools)"]
  ...
  SEARCH["tool_search<br/>Needle 2 + BM25 fallback"]
...
Core synthesizes built-in tools without spawning external processes:
...
| `usage` | current or requested provider/model | Usage windows, remaining quota, and reset times |
...
Examples:
- `spider.crawl` — crawl a URL
- `exa.search` — web search
- `shadow.screenshot` — screen capture
- `ryu_api.get_agents` — Self-API tool
