...
description: "How agents connect to Ryu via the Agent Client Protocol: subprocesses, Gateway egress, the MCP bridge, tools, permissions, and config probes."
tags: ["acp", "agent", "integration", "subprocess", "claude-code", "codex", "gemini", "pi", "byo", "tool-loop", "permissions"]
---

ACP (Agent Client Protocol) lets Ryu run agent subprocesses. Each agent runs in its own process and
streams events back to Core in the Vercel AI SDK format. It gets a full tool loop for reading files,
running commands, and editing code.
...
|---|---|
| `tool_search` | Discover tools and Agent Skills by query — Needle 2 by default, with BM25 fallback or semantic opt-out |
...
For new and auto-installed routable ACP agents, the base-URL and subscription-preserving routing
paths start **on by default**, and the MCP bridge exposes the agent's current enabled tool and skill
scope. An explicit per-agent `false` preference is the direct-egress or bridge opt-out. Agents that
use a provider-native protocol in the no-interception row remain a genuine chat-routing bypass;
their Ryu tool calls are still governed.

## The BYO agent path

For an agent you add yourself (not in the curated registry), Core offers a generic per-agent
gateway-routing toggle. It starts enabled for routable agents; when it is not explicitly disabled:
...
The default already routes this agent through the Gateway. To make its model egress direct, persist
an explicit opt-out:

```bash
# Opt out of gateway routing via preferences
...
ACP agents choose their own names for the most permissive session mode. In the desktop app,
options such as `bypassPermissions`, `danger-full-access`, `Full Access`, `YOLO`, or an equivalent
approval-bypass label are treated as full-access-equivalent. Selecting one first opens a confirmation
warning that explains the file, terminal, internet, and prompt-injection risks; the mode is not
saved until the user confirms. This warning is a client-side guardrail, not a replacement for the
agent's permission protocol or Core/Gateway policy enforcement.
...
Continue with [Integrate with Ryu](/docs/extend/integrate), [BYO agents](/docs/extend/integrate/byo-agent), [agent sync](/docs/gateway/agent-sync), [Gateway for any agent](/docs/gateway/gateway-for-any-agent), and [agent teams](/docs/core/agent-teams).
