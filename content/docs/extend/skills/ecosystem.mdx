...
3. **Loading phase:** The model calls `skills.load` with the skill id. Core loads the full
...
Hooks use the same effective scope order as managed MCP and skill configuration:
...
1. `skills.list` — returns all installed skills (name + description only)
2. `skills.load` — loads a specific skill's full body
...
| **Load** | Model calls `skills.load` → full body injected |
...
Continue with [Skills](/docs/extend/skills), [skill catalog](/docs/extend/skills/catalog), [skill authoring](/docs/extend/skills/authoring), [skill publishing](/docs/extend/skills/publishing), and [Agent Skills extension](/docs/extend/develop/extensions/agent-skills).
