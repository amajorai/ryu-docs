...
[`amajorai/ryu-sdk`](https://github.com/amajorai/ryu-sdk) hub. The main runtime repository does
not carry the language-binding projects; it keeps only the client packages needed by its CLI and
host surfaces.
...
| Python | Rust-kernel surface | [`ryu-sdk` on PyPI](https://pypi.org/project/ryu-sdk/) |
| C# | Rust-kernel surface | [`Ryu.Sdk` on NuGet](https://www.nuget.org/packages/Ryu.Sdk) |
...
The package contains the generated `ryu_sdk` module and the native library for the published
platform wheel. To regenerate and test the binding from the hub, run:
