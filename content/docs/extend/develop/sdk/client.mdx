...
`createRyuClient(options)` returns a `RyuClient` with four typed namespaces - `agents`, `sessions`,
`harness`, and `spaces`. The only required option is `baseUrl`; pass a `token` for authenticated or
remote nodes.
