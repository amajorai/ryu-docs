...
description: "Declare a messaging-platform bot adapter (Telegram, Slack, WhatsApp Personal, WhatsApp Business, Discord) as a Runnable"
---

A **Channel** is one of the eight [Runnable kinds](/docs/start-here/architecture/runnable-model).
It binds a messaging platform - Telegram, Slack, WhatsApp Personal, WhatsApp Business, or Discord - to a Core session, so an
inbound chat message becomes one agent (or [group](/docs/core/agent-teams)) turn and the reply is
...
- **Session path** (recommended) - when the bot config carries an `agent_id` or `team_id`, the
  ...
  Core to Gateway so every call stays governed.
...
```mermaid
flowchart LR
  P[Telegram / Slack / WhatsApp Personal / WhatsApp Business / Discord] -->|inbound| C[Channel adapter<br/>apps/gateway/src/channels]
  C -->|agent_id or team_id set| R[POST /api/channels/run<br/>Core session]
  C -->|neither set| G[Gateway pipeline]
  R -->|Core to Gateway| M[Model + governance]
  G --> M
  M -->|reply| C
  C -->|send_message| P
```
...
The response is plain JSON `{ "reply": "...", "assistantMessageId": "...", "assistantMessageIds": [] }`.
When `idempotencyKey` is supplied, Core retains the completed result for 30 days and rejects
different payloads that reuse the same key. An in-flight retry receives a retryable `503`; the
caller should retry the same request with the same key. Internally a non-empty `team_id` dispatches to
...
[manifest.json manifest reference](/docs/extend/develop/extensions/plugin-json-manifest) for the full
