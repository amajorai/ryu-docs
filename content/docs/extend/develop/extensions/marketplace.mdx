...
A bundle is a small, signed composition package. Put an ordered `bundle.json` in the package folder;
each member references an existing Marketplace `kind` and `id`. The desktop orchestrator calls the
member's normal installer, so an app, plugin, skill, Agent Template, workflow, or MCP server keeps its
own verification and permission boundary. Mark members `required: false` when a missing integration
should not make the whole collection incomplete. Do not put credentials in a bundle and do not nest
another bundle as a member.

Ryu's first-party catalog includes Software Factory, Social Media Marketing, Outreach, and topic
collections based on the [UI Skills catalog](https://www.ui-skills.com/skills). A bundle listing is
metadata and composition; it does not auto-enable the members or bypass node lifecycle controls.
...
directory. Following is personal account state: it does not grant access to private collections,
nodes, package artifacts, or organization controls. Buyers can use the **Following** filter across
Marketplace catalog tabs to see public listings from the organizations they follow.
...
access boundary; transfer the collection boundary as a separate operation first. A public
collection listing is detached from the old collection during the atomic hand-off so the new
owner does not inherit the old organization's publishing scope.
...
Share a listing's web link to preview its name and package type on Ryu's Prism
background in the Pro badge palette. Link previews use a minimal image without descriptions, borders, or
listing statistics. The full description stays on the listing page and in link
metadata. Long names are shortened to fit the preview.

Other public pages and documentation links use the same layout with their own
page title. The Ryu homepage keeps its logo-and-name image. Social services may
cache an older preview until they refresh the link.
