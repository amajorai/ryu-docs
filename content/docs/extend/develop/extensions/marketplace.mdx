...
<Callout type="info">
  Stripe Connect owns seller onboarding, checkout, recurring invoices, transfers, and payouts.
  Ryu remains authoritative for the GitHub package binding and entitlement. A free package needs
  only the node ACL; a paid package needs the node ACL plus an active entitlement. Expiry blocks new
  installs, updates, and downloads but does not remove an installed package.
</Callout>
