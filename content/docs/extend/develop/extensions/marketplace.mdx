...
The supported package kinds are apps, plugins, skills, agents, workflows, themes, output styles,
...
flowchart LR
  ...
  E --> F[moderation]
  F --> G[Ryu entitlement-gated proxy]
...
entitlement are separate checks: being able to browse a collection does not grant access to an
...
is the billing and access bridge behind that seam. New GitHub-backed records store the repository
...
one-time charge with a seller-defined `maxUpdates` allowance. Recurring invoices refresh the same
Ryu entitlement, while each bounded update is consumed atomically at the GitHub download proxy.
...
### Marketplace Membership opt-in

An **app** with a paid offer may opt into **Ryu Marketplace Membership** with
...
listings do not become Membership-eligible. A one-time or bounded-update item sale never grants
Membership by itself; the publisher can still opt that paid app into the separate recurring pool.

Marketplace Membership is a separate recurring Ryu plan priced at **$9.99/user/month**, with a
one-user minimum; buyers choose the number of billed users on the Individual pricing tab. Existing
recurring Pro, Max, and Teams plans carry the same `marketplaceApps` entitlement; the one-time
Desktop license does not. Buyers see **Included with Membership** before install, and a lapsed
recurring plan produces **Upgrade to use** while preserving any separate direct item license.

For each paid recurring period, Ryu freezes a default **70% publisher pool**. First-use signals are
...
<Callout type="info">
  ...
  owns Ryu's recurring plans, including the $9.99 Marketplace Membership tier. Ryu remains
  authoritative for the GitHub package binding and entitlement. A free package needs only the node
  ACL; a paid package needs the node ACL plus either a direct license or an active qualifying
  recurring plan. A lapsed Membership plan blocks use of an included app but does not remove its
  installed package.
</Callout>
...
| `POST /api/marketplace/membership/usage` | `packages/api/src/routers/marketplace.ts` | Record one idempotent first-use signal for an included app |
| `GET /api/marketplace/membership/publisher-report` | `packages/api/src/routers/marketplace.ts` | Read the active publisher organization's Membership totals |
