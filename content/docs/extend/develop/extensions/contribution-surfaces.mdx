...
These contribution surfaces share one shell skeleton even when the app-owned content differs.
...
```mermaid
flowchart LR
  Manifest["manifest.json\ncontributes[...] "] --> Host["Ryu host\nvalidate + authorize"]
  Host --> Shell["Shared shell\nsidebar + tabs + dock"]
  Shell --> Surface["Rendered surface\nview, companion, or widget"]
```
...
The source paths in the extension guides describe the canonical Ryu monorepo. Public repositories
are projections of those sources, and each projection has a different job:
...
change there. Accepted public-hub changes are carried back to the canonical monorepo and may be
rewritten by a later sync.
