...
8. **Declarative contributions** (composer controls, settings tabs, slash commands) are stored and served at `GET /api/plugins/contributions`.

The same generic registration path feeds every client surface. A manifest may target desktop, web,
extension, mobile, CLI, the terminal UI, Core, Gateway, Island, or a hardware-facing host. An empty
target declaration means “all surfaces”; an explicit surface map is authoritative. Surface support
controls discoverability and rendering, not authorization: the host still checks the contribution's
declared grants and the caller's permissions before executing a sidecar, tool, or bridge method.

<Callout type="warn">
  Every Runnable kind has a handler now. The important security distinction remains: Core activates
...
| `{ kind: "replace", text, fresh_session? }` | Swap the pending text before it reaches the model. On the ACP `context` phase, set `fresh_session: true` to send the replacement through a fresh in-process session; the persisted conversation is replayed, but warm session state is not retained. | `pre_user_turn`, `message_end`, `session_compact`, `context` (ACP plane) |
...
- **`context`** is the context-engineering phase, and it spans two structurally different planes. On
  ...
  contains the exact project-instructions block used for the current turn, so a hook can refresh or
  remove it without parsing the surrounding system prompt. The field is absent when no project
  instruction file is available or Safe Mode suppresses project instructions.
...
The repository's plugin test gate scans every packaged hook body under `apps-store` and
`plugins-store` for these portability hazards. A missing sandbox runtime degrades a hook to
...
plugin. It carries a run id, the agent id, a monotonic transition id, and the process-global
`active_count`. A plugin can watch the `1 → 0` transition and send one notification without polling:
...
```json
{
  ...
  "contributes": {
    "turn_hooks": [
      {
        "id": "tokenmaxxing.delegation-idle",
...
notifies only when the global count reaches zero. It is failure-closed: a notification failure does
not start a timer, create a new agent, or claim that work is still active. Disable the plugin if you
do not want an idle notification; it does not change delegation scheduling or model budgets.
...
`next(ctx)` reaches the remaining middleware hooks. A hook can return without calling `next` to
short-circuit its chain, pass a rewritten event context downstream, or await the downstream
directive and wrap it on the way back out. A failed middleware hook is skipped and the chain below
it continues. Legacy hooks (mode omitted) retain their independent directive behavior.

The continuation is Core-owned and one-shot. Request identity (`conversation_id`, `caller_user_id`,
the resolved agent, and plugin flags) is preserved across rewrites, so a plugin cannot retarget a
downstream hook's tenant or enable another plugin's flag. Middleware code still runs in the same
deny-by-default sandbox and uses the same grant-gated `host` capabilities; this is function-shaped
composition, not ambient in-process plugin code.
...
Extensions that deliberately spend extra context or model budget may expose a user-controlled
“token-maxxing” mode through a contribution or setting. It must be opt-in, scoped to the current
turn or conversation, and explicit about cost. The runtime does not enable it merely because a
plugin is installed.

Hooks should deduplicate work at the narrowest durable scope they own. Use the conversation id and a
stable event or turn key for per-conversation state; use the plugin namespace for storage. Duplicate
hook delivery must be harmless, and a dedupe record must not be shared across users, plugins, or
unrelated conversations. Dedupe prevents repeated work; it does not grant permission or turn an
observation-only phase into a control point.
...
Continue with [Extensions](/docs/extend/develop/extensions), [plugin runtime](/docs/extend/develop/extensions/plugin-runtime), [author workflows](/docs/extend/develop/extensions/author-workflows), [author actions](/docs/extend/develop/extensions/author-actions), and [Core observability](/docs/core/observability).
