...
(`plugins-store/sample-widget/`) and adapt it. It is a zero-dependency Node MCP server, the
...
<Callout type="warn">
  The inline-widget renderer has one gate left: the plugin must be installed, enabled, and hold the
  `widget:render` grant. (It used to sit behind a second, desktop-side experimental toggle; that flag
  is gone and the renderer is on for everyone.) Headless widget render is proven; a running desktop
  render of a widget-bearing tool is not yet fully live-verified.
</Callout>
...
The `sample-widget` manifest is published as a reference package at
`plugins-store/sample-widget/manifest.json`. It is **opt-in**, not in `CORE_DEFAULT_ON`, so
...
That is the whole gate. Enabling the plugin is what makes its widgets render. There is no separate
renderer toggle to flip.
</Step>
</Steps>
