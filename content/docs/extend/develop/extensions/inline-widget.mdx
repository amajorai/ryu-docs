...
An **inline widget** is an interactive UI inside a chat reply. It can be a clickable card, a
checklist, or a sortable data grid.

The widget uses MCP. An **MCP server** exposes a **tool**. The tool result carries data and points
to a `ui://widget/<slug>.html` **skybridge** resource. Core renders that HTML in a **sandboxed
iframe** inside the conversation. The widget reads the tool output and talks to the host over a
`MessagePort`. It cannot access the network.

This guide uses the **template path**. Copy the `@ryu/sample-widget` reference plugin from
[`amajorai/ryu-marketplace`](https://github.com/amajorai/ryu-marketplace/tree/main/plugins/plugins/sample-widget)
and adapt it. It is a zero-dependency Node MCP server. Use it when you already have an MCP server
or want no build step. To author the same thing through the SDK's `defineApp`, see
[Author a Ryu App](/docs/extend/develop/extensions/ryu-apps).

<Callout type="warn">
  A Community widget plugin must be installed, enabled, and approved for both `mcp:server` and
  `widget:render`. The first grant registers its MCP server; the second permits the declared widget
  to render. (The old desktop-side experimental toggle is gone.) Headless widget render is proven;
  a running desktop render of a widget-bearing tool is not yet fully live-verified.
</Callout>
...
| File | Role |
| --- | --- |
| `manifest.json` | Manifest: `mcp_servers` + `contributes.widgets` + `mcp:server` and `widget:render`. The registration and consent gates. |
...
flowchart LR
  ...
  subgraph "Chat"
    G[model calls sample_widget.render] --> B
    B --> E
...
1. The model calls the tool `sample_widget.render`.
...
The manifest declares the MCP server, binds its render tool to the widget resource, and requests the
two grants required for a Community MCP widget. This is the minimal relevant excerpt from the current
`@ryu/sample-widget` manifest:

```json
{
  "id": "@ryu/sample-widget",
  "name": "Sample Widget",
  "version": "0.1.14",
  "runnables": [],
  "permission_grants": ["mcp:server", "widget:render"],
  "mcp_servers": {
    "sample_widget": {
      "command": "node",
      "args": ["server.mjs"],
...
- **`mcp:server`** — reserved registration grant for a Community plugin's stdio MCP server. An
  operator must approve it through the [Gateway grant policy](/docs/gateway/governance).
- **`widget:render`** — lets Core promote this plugin's declared tool result into a widget.
  `tool:call` / `ui:send_message` are **auto-derived** from the tool's `widgetAccessible` flag at
  emit time; you never list them here.

<Callout type="info">
  The `tool_id` is the **runtime** id, formed as `<mcp_servers-key>.<toolName>`. Here `sample_widget`
  (the server key) joined with `render` (the tool name from `tools/list`) gives
  `sample_widget.render`. The manifest `uri` and the tool's `_meta.outputTemplate` must be the
  **same** string, and the server must serve that uri from `resources/list` + `resources/read`.
</Callout>

The `@ryu/sample-widget` manifest is published as a reference package at
[`amajorai/ryu-marketplace`](https://github.com/amajorai/ryu-marketplace/tree/main/plugins/plugins/sample-widget).
It is **opt-in**, not in `CORE_PREINSTALLED`, so install/enable it before the widget can render.
...
(e.g. the plugin lacks the `widget:render` grant). Return `isError: false` (the default), because **an error
result emits no widget.**
...
`openai/widgetAccessible: true` **and** the target tool is widget-accessible on that server. If
provenance fails, it rejects. Degrade gracefully rather than break the UI.
...
<Cards>
  <DocCard href="/docs/extend/develop/api-reference/core/widgets/widget_state" />
  <DocCard href="/docs/extend/develop/api-reference/core/widgets/widget_call_tool" />
  <DocCard href="/docs/extend/develop/api-reference/core/widgets/widget_follow_up" />
</Cards>
...
```bash
git clone --depth 1 https://github.com/amajorai/ryu-marketplace.git
cp -r ryu-marketplace/plugins/plugins/sample-widget ~/.ryu/plugins/my-widget
```

Open `manifest.json` and **change the `id`**. The reference uses `@ryu/sample-widget`;
a second installed copy keeping that id collides with the reference package. Pick
...
- `contributes.widgets[].tool_id` → `checklist.show`
...
is discovered and shows up in `GET /api/plugins` as a disabled record. Before enabling a Community
copy, approve the reserved `mcp:server` request through the
[Gateway grant policy](/docs/gateway/governance). Enable it so the live record holds both
`mcp:server` and `widget:render` and its `contributes.widgets` entry goes live:
...
and loopback are rejected). See [Bundle Runnables with manifest.json](/docs/extend/develop/extensions/plugin-json-manifest#5-install-and-enable).

That is the whole gate. Approved server registration plus widget consent makes the tool render; there
is no separate renderer toggle to flip.
</Step>
</Steps>
