...
This guide walks the **template path**: copy the `@ryu/sample-widget` reference plugin
(`plugins-store/plugins/sample-widget/`) and adapt it. It is a zero-dependency Node MCP server, the
lowest-level way to author a widget, and the one to reach for when you already have an MCP server or
