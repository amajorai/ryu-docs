...
This guide uses the **template path**. Copy the `@ryu/sample-widget` reference plugin
(`plugins-store/plugins/sample-widget/`) and adapt it. It is a zero-dependency Node MCP server.
Use it when you already have an MCP server or want no build step. To author the same thing through
the SDK's `defineApp`, see
...
`plugins-store/plugins/sample-widget/manifest.json`. It is **opt-in**, not in `CORE_PREINSTALLED`, so
install/enable it before the widget can render.
...
```bash
cp -r plugins-store/plugins/sample-widget ~/.ryu/plugins/my-widget
```
