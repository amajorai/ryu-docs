...
The bundled plugins in `plugins-store/` are the worked reference: `plugins-store/double-check/`
returns a `note`, `plugins-store/goal/` returns a `continue`, and `plugins-store/advisor/`
