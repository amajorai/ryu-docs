...
| `permission_grants` | Ask the Gateway for host capabilities such as MCP, sidecar, storage, or model access. | A declaration is not approval; enable fails closed when a required grant is denied. |
...
| `surfaces` / `targets` | State whether the extension supports desktop, web, mobile, CLI, and other hosts. | An explicit `surfaces` map wins over legacy `targets`; absent support is not the same as “all” once a map is present. |
