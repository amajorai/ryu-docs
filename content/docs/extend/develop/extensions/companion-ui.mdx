---
title: "Build a Companion UI"
description: "Use Ryu's shared theme, controls, and blocks when building an app satellite."
tags: ["companion", "apps", "ui", "design system", "theme", "blocks"]
---

A Companion is an app surface hosted by Ryu. Its domain model and data adapter belong to the
app, but generic interaction and visual language belong to Ryu. Start with the shared design
system so the surface behaves like the rest of the product: the same focus treatment, keyboard
semantics, loading states, spacing tokens, and light/dark theme contract.

Navigation follows the same ownership rule. A Companion must not grow a competing left sidebar.
Declare `sidebar_sections` in the app manifest for app-owned record lists; Ryu gives the enabled
app one icon tile in its **Apps** shelf and renders those sections in the host sidebar. The
standalone Ryu App window renders the same Ryu sidebar beside the Companion. Keep only domain
detail panes, inspectors, graphs, and canvas rails inside the app. `sidebar_buttons` is reserved
for plugin-level navigation and is not an app's primary display.

Companion typography comes from the host tokens: use `font-sans` for interface copy,
`font-heading` for display headings, and `font-mono` only for code or technical values. Ordinary
labels, headings, status text, and descriptions should stay in the normal interface face. Do not
introduce app-local system, platform, or decorative font stacks.

This is the UI half of [Author a Ryu App](/docs/extend/develop/extensions/ryu-apps). The sidecar
still owns domain state and reaches the host through the app bridge; the Companion should not
rebuild Core capabilities or call a sidecar directly from the browser.

## The three shared seams

Use the host's `spaces` methods when an app needs searchable documents. The
`spaces:docs` grant allows app-owned documents while the caller's Space permissions,
document visibility, and tenancy still apply. `spaces.search` uses Core retrieval;
an app must not replace an access denial with a local cached answer. Copying domain
content into a Space should be an explicit user action with clear sharing context.

| Need | Use | Own locally |
| --- | --- | --- |
| Theme tokens and fixed app frame | `@ryu/ui/app-ui.css` | Domain layout and art |
| Buttons, fields, dialogs, badges, loading | `@ryu/ui` or the companion control exports from `@ryu/blocks` | Domain-specific form meaning |
| A repeated product section | `@ryu/blocks` | The bridge adapter that supplies data and actions |

Keep graph geometry, canvas rendering, slide layers, and other domain art local. Move generic
cards, tabs, empty states, error surfaces, controls, and interaction states onto the shared seams.
If two surfaces need the same domain section, add a composed block instead of copying its JSX into
the next app.

## Install the shared substrate

Import the fixed Ryu App UI stylesheet once from the app's stylesheet, after Tailwind:

```css
@import "tailwindcss";
@import "@ryu/ui/app-ui.css";
```

The theme exposes the semantic tokens used by Ryu UI, including `background`, `foreground`,
`card`, `muted`, `primary`, `destructive`, `border`, `input`, `ring`, sidebar/chart roles, and
the status colors. Do not copy the token block into an app stylesheet or maintain a second
light/dark palette.

Subscribe to the host's live theme before mounting the app:

```tsx
import { subscribeCompanionTheme } from "@ryu/app-host/companion-theme";

subscribeCompanionTheme();
```

The host sends the resolved palette, not just a preset id, so built-in, custom, and installed
theme palettes all render the same inside the Companion. The live snapshot also carries the
shared radius, spacing unit, Card padding, UI zoom, font stacks, and the host's light/dark class.
It mirrors the appearance controls that are meaningful inside an app or plugin surface: pointer
cursor, animation master switch, popup/dialog backdrop treatment, inverted popup backgrounds,
page background customization, and related accessibility state. A Companion never needs to read
the host's storage or infer the active mode from the operating system. The host reapplies the
snapshot when any of those settings change, including while the app is already open.

Date and time display crosses the same boundary. Use `formatDate`, `formatTime`, or
`formatDateTime` from `@ryu/ui/lib/timezone` for timestamps instead of browser-local
`toLocale*` calls. The host projects the selected display timezone and locale as
`--ryu-timezone` and `--ryu-locale`; `RyuAppShell` subscribes once so timestamp labels repaint
when Desktop's Date & time or language setting changes. Date-only values should remain calendar
values, not be converted into a different day by a timezone offset.

Mark the mounted root as an App UI v1 surface. `v1` is the stable contract version, not a partial
implementation label. This is the same contract used by the first-party apps and lets the host and
the repository checker distinguish a Ryu app surface from an arbitrary webview:

```tsx
import { markCompanionAppRoot } from "@ryu/app-host/companion-theme";

const root = document.getElementById("ryu-plugin-root");
markCompanionAppRoot(root, { surface: "standard" });
```

The entrypoint must also compose the mounted tree through `RyuAppShell` from
`@ryu/blocks/companion/app-ui`; importing the stylesheet alone is not a port.

## Use the fixed app vocabulary

For app-level composition, use `@ryu/blocks/companion/app-ui`:

```tsx
import {
  RyuAppEmpty,
  RyuAppList,
  RyuAppListItem,
  RyuAppShell,
  RyuAppToolbar,
} from "@ryu/blocks/companion/app-ui";

<RyuAppShell>
  <RyuAppToolbar title="Projects" />
  <RyuAppList aria-label="Projects">
    <RyuAppListItem
      selected={selectedId === project.id}
      subtitle="Updated today"
      title={project.name}
    />
  </RyuAppList>
  {projects.length === 0 ? (
    <RyuAppEmpty description="Create a project to get started." title="No projects" />
  ) : null}
</RyuAppShell>
```

The available roles are intentionally small: shell, toolbar, main, section, list, list section,
list item, detail, form, field, empty, and actions. This is the Ryu equivalent of an opinionated
List/Grid/Detail/Form surface: agents choose the product data and behavior, while Ryu chooses the
visual grammar. If a pattern is missing, add it to the shared package instead of designing a local
replacement.

The helper is safe in standalone development, where no host bridge is present. In a hosted,
null-origin frame it applies only validated CSS custom properties and reserved appearance state
supplied by Ryu. A missing host bridge leaves the bundled light/dark defaults in place.

## Use shared controls

Use Ryu controls directly, or use the companion control seam when a satellite needs the small
surface-neutral collection:

```tsx
import {
  Button,
  Input,
  Textarea,
} from "@ryu/blocks/companion/controls";

export function SavePanel({ onSave }: { onSave(): void }) {
  return (
    <form onSubmit={(event) => { event.preventDefault(); onSave(); }}>
      <Input aria-label="Project name" placeholder="Project name" />
      <Textarea aria-label="Description" placeholder="What is this for?" />
      <Button type="submit">Save</Button>
    </form>
  );
}
```

Use `variant="secondary"`, `variant="outline"`, `variant="ghost"`, or
`variant="destructive"` for intent. Use the shared `Dialog`, `Switch`, `Checkbox`, `RadioGroup`,
`NativeSelect`, `Badge`, `Empty`, and `Spinner` components for the corresponding interaction;
do not recreate their CSS or keyboard behavior in the app.

## Compose a block when the behavior repeats

Blocks should receive data and callbacks rather than know how to fetch an app's sidecar:

```tsx
<MonitorsView
  monitors={monitors}
  selectedId={selectedId}
  onSelect={setSelectedId}
  onSave={(input) => void saveMonitor(input)}
/> 
```

The desktop and Companion adapters can then share the presentation, validation, empty state, and
control geometry while keeping their bridge and lifecycle code separate.

## Review checklist

- The stylesheet imports `@ryu/ui/app-ui.css`; it does not duplicate Ryu's token block.
- The entrypoint calls `subscribeCompanionTheme()`.
- The entrypoint calls `markCompanionAppRoot()`.
- The entrypoint mounts its app through `RyuAppShell` from `@ryu/blocks/companion/app-ui`.
- Generic buttons, fields, dialogs, tabs, badges, loading, and empty states use `@ryu/ui` or
  `@ryu/blocks`.
- App-level layout uses `@ryu/blocks/companion/app-ui` instead of a local shell vocabulary.
- App primary navigation is declared through `sidebar_sections`; the host provides one Apps-shelf
  tile and the Companion does not render a second application sidebar.
- The app does not create a second query shim, toast implementation, theme subscriber, or sidecar
  process helper.
- Domain-specific canvas, graph, media, and editor rendering remains local and is not forced into
  a generic block.
- The surface is checked in both light and dark host themes, including focus and disabled states.
- The surface is checked with a custom palette, non-default contrast/radius/zoom, and a live
  appearance change while the Companion remains mounted.
- Timestamp displays use `@ryu/ui/lib/timezone` and follow a live host timezone/locale change.

## Follow the host theme

Companion toolbars, lists, forms, and dialogs use the host's semantic colors and
shared controls. App-local aliases should reference those tokens rather than define
another light or dark palette. Avoid redefining shared names such as `--muted` for
text: `--muted` is a surface color and `--muted-foreground` is secondary copy.

Use the shared dialog and menu parts for confirmation and selection flows. Menu
labels belong inside their corresponding menu group. Shared dialogs provide focus
management, dismissal, and responsive behavior. Status labels use `--status-success`,
`--status-warning`, `--status-info`, and `--status-destructive`; their fills continue
to use the base status colors.

Drawing paper, slide artwork, and user-selected category colors may retain their
own palettes. Surrounding controls still follow the host theme.

Use `Checkbox` for boolean choices, `RadioGroup` for exclusive choices, and `Slider`
for ranged values. A text-field `Input` is not a substitute for these controls. Shared
Base UI triggers compose a child through `render`; the older Radix `asChild` prop
does not attach the required behavior.

For an existing asynchronous action that needs a confirmation gate, use
`useConfirmDialog` from `@ryu/ui/hooks/use-confirm-dialog.tsx` and render its
`confirmationDialog` in the same component. Await `confirm(message, options)` before
calling the domain operation. It returns `false` on cancellation or unmount; options
can supply explicit confirm/cancel labels, a description, and a destructive variant.
The shared AlertDialog owns focus, Escape, and responsive layout.

## Transcription with timing

A Desktop Companion with the approved `media:transcribe` grant can request
`window.ryu.media.transcribe({ audio, filename, detailed: true })`, where `audio`
is an audio data URL. A detailed result contains `text` and engine-provided
`segments`, each with `startMs`, `endMs`, and `text`. Segment times are milliseconds.
When the engine supplies no timestamps, the segment array is empty.

Detailed results may also include `words`, using the same millisecond fields.
These are retained only when the speech provider returns explicit word alignment;
an absent or empty array means word timing is unavailable. Preserve the original
segments for normal captions, and use the word array only for word-level features.

Calls without `detailed` keep the existing plain-text result. Treat a plain-text
response from an older host as a supported fallback; do not fabricate word
alignment. The shared Core client also exports `transcribeAudioDetailed` for
trusted clients that need this existing Core response metadata.

## Polling without overlapping requests

Use the shared `useQuery` hook from `@ryu/ui/hooks/use-query.ts` for periodic
read-only companion data. Automatic polling waits for the current request to
settle, pauses when the document is hidden, and refreshes when it becomes visible.
An explicit `refetch()` still requests newer data, and late results from an older
request or a closed view are ignored.

Keep durable jobs in the owning service. Pausing a view’s polling does not stop
an agent run, recording, export, or scheduled job.

### Embedded tab visibility

The shared query hook also pauses automatic polls when a browser-based host hides
its sandboxed frame for an inactive tab. It resumes when the frame is visible
again, without remounting the companion or resetting its draft state. This uses
the browser's intersection signal and requires no additional plugin permission.
Manual refreshes and already-started work retain their normal behavior.

### Awaiting a refresh after a mutation

`await query.refetch()` waits for that refresh to settle. Use it after a mutation
when the action should remain busy until its refreshed list arrives. Read errors
are exposed through the query result, and the refresh promise resolves after the
error state is recorded. Automatic interval polls still avoid overlapping reads.

### Document replacement and reconnecting

Appearance updates keep the running companion document and its draft state.
Replacing the UI bundle or changing its mount context establishes a fresh sandbox
bridge. Desktop scopes cached bundles to the selected node and credentials, and
cancels an unused bundle request when its query is abandoned.

Desktop realtime joins belong to their mounted companion document. Closing or replacing
that document ends its active connections and rejects pending joins. A credential
response arriving afterward cannot open a connection for the closed document.

Contribution metadata is shared by consumers using the same node and credentials.
Switching the signed-in identity starts a separately scoped read; an abandoned
contribution request is cancelled and its late response cannot replace the new
identity's metadata.


Fine-tuning keeps a pending job-list read from overlapping later automatic polls
and pauses those polls while its retained app view is hidden. Returning refreshes
the list and preserves configuration drafts. The selected run's live progress
subscription remains active while mounted; closing or replacing it rejects late
frames. Training remains owned by its service.


Video Studio pauses export-status and expanded source-analysis polling in hidden
app views. Pending reads do not overlap, and an older status response cannot
remove a newly queued export. Starting source analysis again resumes progress
updates after a previous run completed. Playback and service-owned render and
analysis jobs continue independently of these view refreshes.


Video Studio retains preview media only while the current timeline references it.
Projects sharing a source can reuse its loaded preview or pending read. Switching
to a project that no longer uses that source releases its preview URL and stops
requesting further media chunks. Opening it again reloads the source as needed.


Research campaign lists and details wait for slow automatic reads and refresh
when the app becomes visible again. Detail results stay tied to the selected
campaign. Research elapsed-time displays, Social queue countdowns, Social calendar
clocks, and the idle Timeline clock pause in hidden views and catch up on return.
Timeline keeps its live clock active during user-requested playback.

Use `useViewClock` from `@ryu/ui/hooks/use-view-clock` for passive wall-clock
labels. Its first argument is the tick interval in milliseconds; its optional
second argument keeps ticking in the background for active playback.


Workflow recording counters and approval-wait status reads pause in hidden app
views and avoid overlapping requests. Start and Stop invalidate older counter
reads, so a delayed status response cannot restore a stopped recording. Recording
and workflow execution remain owned by the host. On return, status refreshes and
the run result panel follows the latest completion or approval state.


Video Studio generation history updates independently of Gateway budget and audit
reads. While jobs are active, budget information refreshes at most about every
30 seconds, with a refresh when jobs finish or the app becomes visible again.
Hidden views pause these automatic reads; generation itself continues in its
owning service.
