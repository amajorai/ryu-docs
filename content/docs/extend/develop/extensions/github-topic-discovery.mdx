...
For reviewed first-party marketplace collections, the manifest's icon and optional
`iconDither` are rendered into light and dark theme tiles above the App or Plugin
name, with the complete icon/name cell centered in the generated catalog README.
Short metadata columns such as status marks, layers, and versions are centered too;
descriptions remain left-aligned for scanning. Keep the manifest icon stable so the
listing does not change appearance between mirror refreshes.
