...
[the SDK reference](/docs/extend/develop/sdk), and for every manifest field see
[the manifest.json manifest](/docs/extend/develop/sdk/plugin-api).
...
<Callout type="info">
  ...
  starter agent has something to call. From the repository root, `bun dev` starts the product
  development stack (Web, server, Core, Gateway, Desktop, and Fumadocs); satellite/native/CLI
  surfaces have dedicated commands, including `bun run dev:cli` for the interactive terminal
  client.
</Callout>
...
plus manifest signing). Create the immutable `.ryupack` release first, then submit its repository
URL with `bunx ryu github-publish <url>`; private repositories may ask the seller to install the
Ryu GitHub App once.
That flow lives on [the marketplace authoring page](/docs/extend/develop/extensions/marketplace), not
here.
