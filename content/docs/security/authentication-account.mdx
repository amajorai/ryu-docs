...
The website also renders a QR for this request. On an already-signed-in phone, open Ryu Mobile's
**Settings → Scan sign-in QR**, scan the website QR, verify the surface, device label, and matching
code, then tap **Approve sign-in**. The QR is only a transport for the expiring user code: scanning
does not approve the request, and the mobile app will not show a request belonging to another signed-in
account. The visible code remains available when scanning is not convenient.
...
page, then enter the six-digit code sent to your inbox. On the next step, choose a new password; the
code and password are sent together when you finish. Codes expire after five minutes; use **Resend
code** if the first message does not arrive.
