...
Updating, disabling, or revoking an organization API key requires a recent identity confirmation
in the current session. This protection also applies to direct authentication API requests.
Managing your personal keys does not require this additional organization confirmation.

## Hosted support sessions

Cloud support access is a separate, user-granted control-plane session. It is off by default. Choose
one or more of `billing`, `subscription`, `organization`, `sync-status`, `channels`, and
`marketplace`, and access ends when the grant expires, is revoked, or is replaced. Omitting the scope
list uses every scope in the active grant; an empty or unknown list is rejected. The support actor
must be on Ryu's support allowlist and complete the staff identity step-up before the handoff.

Support sessions use Better Auth's native session engine and retain the actor in `impersonatedBy`.
Every session is tied to one live grant and one audit record with a reason. A session can call only
the reviewed read paths for its granted scopes. It cannot mutate organization state or credentials,
mint API keys or JWTs, use OAuth/MCP/Core delegation, or read prompts, messages, conversations,
documents, channel secrets, or channel system prompts. Revoking or replacing access takes effect on
the next request, even when a browser has a cached session. Cookie clients use the native stop
endpoint to restore the original staff session; a bearer-only stop ends the restricted session and
asks the client to select its retained staff credential, without minting or reconstructing an actor
token.
...
bounded, redacted control activity with the actor, agent, target, outcome, and request/session
correlation when available. Ryu does not currently claim full parity with Better Auth's hosted
Sentinel risk analytics; deployments that need retention, export, alerting, or additional risk rules
should build those as explicit Ryu policy features rather than treating an audit row as a risk
verdict.
