...
- **The LLM never receives raw secrets.** Credentials live in [Passport](/docs/standalone/passport) and are consumed at the execution boundary, not placed in the prompt. See [Credential and environment scrubbing](/docs/security/credential-scrubbing).
...
- **Plugins are contained unevenly, on purpose.** A plugin's hook, adapter, and UI code runs in a real sandbox; a plugin's sidecar or MCP server runs with Ryu's own privileges. Which one you get depends on what the plugin contributes, so publisher trust is what governs the uncontained half. See [Plugin isolation](/docs/security/plugin-isolation).
