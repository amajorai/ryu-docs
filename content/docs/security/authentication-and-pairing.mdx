...
credential, a node bearer is not a hardware token, and an MCP grant is not an organization-wide
permission slip.
...
not a promise that hosted model usage has no cost. On a computer, the guest action checks for a
...
flowchart TD
  ...
  Method -->|"Email, password, magic link, or OTP"| Email["Email verification"]
  Method -->|"Use a passkey"| Passkey["WebAuthn assertion"]
  Google --> Auth["Better Auth account flow"]
  Email --> Auth
  Passkey --> Auth
  Auth --> TwoFactor{"Does the account require 2FA?"}
  TwoFactor -->|"Yes"| TOTP["Verify TOTP or backup code"]
  TwoFactor -->|"No"| Session["Create account session cookie"]
  TOTP --> Session
