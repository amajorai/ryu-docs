...
| **Plugin provider grant** | A plugin's remote MCP connection | One declared provider resource | Core OAuth broker and Passport |
...
Browser clients do not put node or user credentials in a WebSocket URL. They first exchange their
normal HTTP credentials for a short-lived, single-use ticket bound to the exact room or extension
path, then use only that opaque ticket during the upgrade. Core rejects replayed, expired, rotated,
or mismatched tickets and closes an established user session when its validated JWT expires.
