...
The managed provider is always Gateway-routed so egress stays governed and metered. In local mode
the model is the same GGUF that powers the `@ryu/engines` chat engine.
...
| whisper.cpp | `ggml-base.en.bin` | 8090 | Speech-to-text (STT) |
| parakeet | `parakeet-v3-int8` (ONNX, in-process) | - | Alternative STT (feature-gated) |
| sd.cpp | `sdxl_base_1.0_Q8_0.gguf` | 8083 | Image generation |
| ryutts (TTS) | Kokoro 82M v1.0 (`kokoro-v1.0.onnx`), voice `af_heart` | 8085 | Text-to-speech (default engine) |
| outetts | `OuteTTS-0.2-500M-Q4_K_M.gguf` | - | TTS (alternate) |
...
<Callout type="info">
  **STT defaults.** `whisper.cpp` is the STT engine that works out of the box - the `ggml-base.en`
  model is downloaded on first run. Parakeet is installed ready but its inference is gated behind
  the off-by-default `voice-parakeet` build feature. See [Voice engines](/docs/desktop/engines/voice-engines).
</Callout>
...
The `gemma-4-E2B-it-Q4_K_M` default in these apps is the same local GGUF as the chat engine - it
...
<Cards>
  ...
  <DocCard href="/docs/benchmark/local-models" />
</Cards>
