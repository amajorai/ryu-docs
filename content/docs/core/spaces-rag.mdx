...
The desktop Space home is a shared-context workspace for both people and agents. Open a Space to
see its name, a Word-like left navigation rail, quick templates for creating a page or database,
and a small set of the most recently updated Markdown pages. Each recent row includes a paper-like
preview of the source without opening the editor; selecting one opens the full page.

The recent-page area includes **Recent**, **Pinned**, and **Shared with Me** tabs. Search filters the
recent rows and the file index together, so one query can take you from a page preview to the
underlying document record.

Below the previews, **All files** keeps every item in the Space in one searchable table. Pages,
databases, whiteboards, uploaded files, and documents owned by installed Ryu apps remain in this
same index, with their type, size when available, last modified date, and search reach. The table's
search filters the visible rows locally; the separate Space search still runs retrieval across the
stored document text. File rows that only have a name and type indexed keep that status visible in
the table without changing the fact that the stored file can still be opened.

The file drop zone, retrieval-mode, import/export, backup, and semantic-search controls are
available from the shared-context home. Dropped files go through the node's configured extraction
and embedding path, while creating or selecting a document continues to use the same Space
document routes and permissions described by the API reference.
...
- `POST /api/retrieval/index` adds a stable-id chunk to retrieval. Its default
  `memory` source is encrypted and replaces the same-id user-level memory on a
  repeat request; use `POST /api/memory` when you need explicit scope, category,
  importance, or tags. Pass `source: "space"` and a `space_id` for a Space chunk.
