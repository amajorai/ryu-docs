...
The MCP registry (`apps/core/src/sidecar/mcp/mod.rs`, `McpRegistry`) is Core's list of tool servers.
It speaks the Model Context Protocol, combines built-in and user-configured servers, and checks an
agent's allowlist before each call. Chat, ACP and OpenAI-compatible agents, workflows, monitors,
recipes, and programmatic tool calls all use this registry.

This page describes **Core calling another MCP server**. It is the opposite direction from [Ryu
MCP](/docs/extend/mcp), where an external MCP host calls Ryu. The public website also has an
anonymous read-only `/mcp` endpoint; it cannot reach this registry or a private node.
...
Catalog MCP installs accept `dryRun: true`. `POST /api/mcp/catalog/install` then resolves and
validates the command or remote bridge, reports the disabled server entry it would write, and does
not modify `mcp.json`, create a plugin lifecycle record, or warm an npm package.
...
An organization can also apply a signed Fleet assignment with `kind: "mcp"` to block a server on
enrolled nodes. A blocked server is removed from summaries and tool discovery, and the dispatch
chokepoint rejects a direct call even if a stale user-owned config still names it. This node policy
is separate from the per-agent allowlist and from the caller's organization/resource permissions;
all three layers must allow the action.
...
| `web_fetch` | Authenticated web fetch over HTTPS | Built-in. Uses the user's [Passport](/docs/standalone/passport) connection for the URL's host server-side (acts as the user; the credential never reaches the model) |
| `usage` | Provider/model usage and quota windows | Built-in. Returns remaining/used percentages, window length, prepaid meters, and RFC 3339 reset timestamps; credentials stay inside Core |
...
desktop this is **Device settings** for the selected device; `locked_use` is an
...
### Pre-installed MCP plugins

Ryu also ships manifest-owned MCP plugins alongside its bundled skills. They are
pre-installed and enabled on a fresh node, then started lazily when an
...
|---|---|---|---|
| [Expect](/docs/plugins/expect) | `expect` | Runs browser QA plans against the current agent changes | `npx -y expect-cli@0.1.3 mcp` |
| [Agentation](/docs/plugins/agentation) | `agentation` | Reads and updates visual annotations from a running web app | `npx -y agentation-mcp@1.2.0 server` |
...
the caller's bound [Passport](/docs/standalone/passport) profiles.

```mermaid
flowchart TD
  Call["call_tool_with_identity(id, args, allowlist, user_id, profile_ids)"] --> Vault{Passport consult}
...
1. **Passport consult.** For a bound agent, a call targeting a domain that is `NEEDS_AUTH`
...
On the direct Core tool-call endpoint, `host_conversation_id` is an internal forwarding context, not
a caller-selected vault identity. A request carrying a managed user JWT may name only a conversation
that user owns; visibility or collaboration on another user's conversation does not delegate that
owner's user-scoped vault secrets. Core-to-Gateway forwards carry a process-local proof; a bare node
bearer cannot select a host conversation on a bound node. Core also validates the portable
conversation-id grammar before it can reach shell-backed agent startup.
...
- **One chokepoint, both planes.** Tool execution on the ACP and openai-compat planes is allowlist-gated
  and audited through this same dispatch, closing the ACP/registry tool-egress bypass. The
...
Each tool-discovery request checks up to four servers concurrently, retaining each server's
results and skipping failed servers as before. A slow server does not prevent
other discovery slots from making progress. An explicitly empty agent allowlist
returns no tools without starting server discovery; normal allowlist and
credential checks still apply to non-empty discovery requests.
