...
| `shadow` | Screen/audio/input capture + semantic search | HTTP to the Shadow sidecar on `:3030`. Cross-platform; reports unavailable when Shadow is not running |
...
`expect__<tool>` and `agentation__<tool>`. Disable either plugin from the App
store when its local bridge is not wanted.

<Callout type="warn">
  Ghost and Shadow are cross-platform and many built-ins are opt-in or require a sidecar/key. A server
