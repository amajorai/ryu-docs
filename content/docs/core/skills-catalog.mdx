...
| `POST /api/skills/catalog/install` | Install `{ id, source?, promptForTargets?, targetIds?, rememberTargetIds? }` into the universal layout, hot-reload, and mark active |
...
package integrity. A node-backed desktop also exposes **Scan with agent**. It
sends a bounded, read-only evidence snapshot to the reviewer selected in
**Gateway → Guardrails → Catalog scanner**; the agent's narrative is shown
separately and cannot change the numeric score. Published skill files are
