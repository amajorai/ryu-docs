...
`skills.search` ranks through the shared `ToolRanker` — BM25 by default, or the semantic ranker
when `tools.active_ranker` selects it — so skill discovery and tool discovery use one ranking
strategy and one preference. See [Unified tool catalog](/docs/core/unified-tool-catalog#ranking).
