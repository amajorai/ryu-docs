...
description: "Browse and install Agent Skills from a federated registry catalog, with a per-agent allowlist and both-plane injection."
...
[skills.sh](https://skills.sh), vendor GitHub taps, browse.sh, ClawHub, LobeHub,
Ryu Marketplace, and user-added Claude marketplaces, installs into the universal
Agent Skills layout shared with Claude Code, tracks which skills are active, and
injects the filtered skill block into every request on both chat planes.
...
[Skills](/docs/surfaces/desktop/skills).
...
On the desktop Skills detail view, **Health** runs a skill-specific deterministic
scorecard over source trust, published audits, maintenance, documentation, and
package integrity. A node-backed desktop also exposes **Audit with agent**. It
sends a bounded evidence snapshot to the reviewer selected in
**Gateway → Guardrails → Health audit agent**; the advisory score, evidence,
recommendations, and limitations are shown
separately and cannot change the static score. Published skill files are
untrusted evidence, so the reviewer is instructed not to follow instructions
found inside them.
...
On desktop, installing a catalog skill can also offer it to supported agent clients
on the same computer. Ryu reads the target registry and marks detected agents first;
project-only targets are shown but cannot be selected for global distribution.
Choose **Use with agents** from an installed skill's detail view or Library card,
select one or more targets, and optionally remember that selection for future
catalog installs. An empty selection keeps the skill available in Ryu only.

The copy is performed by Core, including the skill's supporting files. If a target
already contains a different copy, Ryu leaves that destination unchanged and
reports the conflict. The browser catalog remains read-only and does not expose
this local distribution action.

When `promptForTargets` is true and no saved preference exists, the install route
returns `409` with `skill_targets_required`; the desktop flow opens the target
picker and retries with the user's explicit choice. Target selection is a local
filesystem operation and does not grant the target agent access to Ryu credentials.
...
`skills.search` ranks through the shared `ToolRanker` — Needle 2 by default, BM25 as the
deterministic fallback/opt-out, or the semantic ranker when `tools.active_ranker` selects it — so
skill discovery and tool discovery use one ranking strategy and one preference. See
[Unified tool catalog](/docs/core/unified-tool-catalog#ranking).
...
`skills.search`), plus a one-line instruction to call `skills.load` before using a
skill.

The index is **deterministically ordered by skill id**, so which entries fall inside the cap is
reproducible across restarts rather than depending on filesystem enumeration order. Past 20
enabled skills the cut is therefore stable but alphabetical — if a skill must always be visible up
front rather than discovered through `skills.search`, mark it `always-on: true`.

The index is deliberately **query-independent**: it is part of the system preamble, and a preamble
that varied per message would invalidate the provider-side prompt cache on every turn. Narrowing
by relevance is `skills.search`'s job, which the model calls when it needs it.
...
`skills.load`. The **openai-compat plane keeps full injection** — its default fast path
...
`skills.search` and `skills.load` are scoped by the calling agent's allowlist as well as the global
enabled set. The index the model sees and the skills it can reach are therefore the same set. A load
for an id outside the allowlist is refused with the same message as a load for an id
that does not exist, so the response cannot be used to probe for skills the agent is not allowed
to see.

Scoping is resolved from the agent record, so it applies on both agent planes. Autonomous engine
operations that carry no agent configuration (scheduled quests, monitors, workflow steps) run against the
full enabled set: a skill is instruction text rather than a credential, so this scoping is about
keeping an agent focused, not about confidentiality.
...
An agent can write its own skill. The `skills.author` tool
(`apps/core/src/sidecar/mcp/skills_tool.rs`, the same reserved server as `skills.search` and
`skills.load`) renders a structured `SKILL.md` with four fixed sections (Purpose, Procedure,
Failure modes, Verification) into the universal layout at `~/.claude/skills/<slug>/SKILL.md`. The
self-authored skill is immediately discoverable by the existing `skills.search` / `skills.load`
...
<Callout type="warn">
  ...
  `skills.author` is neither listed by `tools()` nor callable: `do_author` returns
