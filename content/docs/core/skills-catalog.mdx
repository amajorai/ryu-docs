...
`skills__search` ranks through the shared `ToolRanker` — BM25 by default, or the semantic ranker
...
front rather than discovered through `skills__search`, mark it `always-on: true`.
...
by relevance is `skills__search`'s job, which the model calls when it needs it.
...
`skills__search` and `skills__load` are scoped by the calling agent's allowlist, not only by the
...
<Callout type="warn">
An agent that carries a restricted **tool** allowlist must include `skills__*` in it, or it cannot
