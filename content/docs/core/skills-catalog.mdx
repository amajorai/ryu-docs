...
| `POST /api/skills/catalog/install` | Install `{ id, source? }` into the universal layout, hot-reload, and mark active |
...
### The allowlist scopes the tools, not just the index

`skills.search` and `skills.load` are scoped by the calling agent's allowlist, not only by the
global enabled set — so the index the model sees and the skills it can actually reach are the same
set. A load for an id outside the allowlist is refused with the same message as a load for an id
