...
It lives in `apps/core/src/identity/` (`mod.rs`, `store.rs`, `source/`, `elicitation.rs`,
`governed.rs`, `health.rs`, `consult.rs`).

For the desktop how-to (creating a profile, importing a cookie, binding it to an agent) see
[Identities](/docs/surfaces/desktop/identities). For the grant + audit half see
[Governance](/docs/gateway/governance).

For user-managed named values such as API tokens, use the separate
[Secret Vault](/docs/security/secret-vault). Identity profiles and Secret Vault values have
different ownership and resolution rules.
...
<Callout type="warn">
  ...
  `web_fetch.get` consumer are unit-tested (9 + 7 tests green) but **not yet live-verified**
...
Remote MCP OAuth and direct Composio connections also carry a separate Ryu-side action ceiling.
It protects accounts that may contain contacts, messages, customer records, or HR details; it is
not a replacement for the provider's OAuth scopes or the Gateway's `identity.read` grant.

| Level | Effect |
|---|---|
| **Risk-based (recommended)** | The default. Known reads flow normally and Ryu's smart approval policy gates risky writes and deletes. |
| **Read only** | Allows reads and previews only. |
| **Write access** | Allows reads and writes, but blocks known deletes. |
| **Full access, including delete** | Allows known reads, writes, and deletes while preserving destructive-action confirmation. |

The connection review dialog appears before authorization in the Desktop Connections surface. Core
stores the choice with the owner and connection binding and checks it at tool dispatch. Unknown
actions fail closed even when a connection is set to Full.
...
flowchart TD
  ...
  Q -->|web_fetch.get| W[ProceedWithCredential: hand secret out-of-band]
...
Composio tool ids (`composio.…`) are skipped here: Composio owns its own connection-required
path (`mcp::composio::detect_elicitation`), and running both would collide.

## The credential consumer: `web_fetch.get`

A consult that only read and dropped the secret could prove the governed path but never use it.
The `web_fetch` builtin closes that seam. `web_fetch.get`
...
Secret Vault's organization, team, node, and user levels do not change this Identity Vault
boundary. A per-domain login remains a profile-bound identity until a separate shared-identity
