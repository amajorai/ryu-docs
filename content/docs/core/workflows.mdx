...
tags: ["workflow", "dag", "orchestration", "durable", "automation", "trigger", "schedule", "dry-run"]
---
...
see [Recipes and the canvas](/docs/surfaces/desktop/productivity/recipes). To author a workflow as a
Runnable in code, see [Authoring workflows](/docs/extend/develop/extensions/author-workflows).

## A run in the desktop

The engine's states become a readable run surface: steps, activity, progress, and tool output stay
visible together while the graph executes.
...
of hand-assembling the compound `<server>.<tool>` id, and the executor reuses the `tool` path so the
...
downstream. Any of `first`, `all`, `quorum` (with `n`), or `percentage` (with `percent`) turns the
...
<Callout type="warn">
  `recipe` and `ghost_action` are cross-platform and no-op with a clear error until the ghost sidecar
  is installed. See [Recipes](/docs/surfaces/desktop/productivity/recipes).
</Callout>
...
tools are skipped. Prompts, agents, skills, delegation, plugins, guardrail checks, webhooks,
notifications, channel sends, recipe replay, ghost actions, and human-in-the-loop gates are also
...
<Callout type="warn">
  The durability and dry-run paths are covered by focused unit and contract tests, but live crash
  and resume and the HITL `awakeable` round-trip have not been exercised against a running Core.
</Callout>

## Catalog and version previews

Installing a workflow template accepts `dryRun: true` and returns the template metadata, body count,
fresh workflow count, and trigger-write plan without creating workflow files or scheduler jobs. The
version restore route also supports the same preview flag when a client needs to show the target
version and undo behavior before changing the current definition.
...
<Callout type="info">
  Saving a Composio trigger reconciles the project's V3 webhook subscription
  against the node's current public ingress URL before the trigger instance is
  created. If the URL or signing secret cannot be made ready, Ryu keeps the
  existing workflow subscriptions instead of replacing them with an unreachable
  configuration. See [Node and presence](/docs/core/node-and-presence).
</Callout>

## Version history

Every workflow keeps an immutable, server-backed source history in the node-local Git repository at
`~/.ryu/source-history/workflows/<workflow_id>.json` (`apps/core/src/workflow/store.rs`). A commit
contains the full `Workflow` JSON plus a short checkpoint label, and the existing generic
`VersionHistory` control still provides the list, diff, and restore experience. Git is the history
primitive here; the old `~/.ryu/workflow-versions/` JSON snapshots remain readable as a migration
fallback for nodes that already have them.

Saving a workflow checkpoints its current definition. A restore records the current definition as a
named undo point, then re-persists the captured definition through the shared `persist_workflow`
write path, so triggers reconcile and `updated_at` is re-stamped. Unlabeled identical saves are
coalesced; explicit labels remain visible as named Git checkpoints. Git history is local and
immutable rather than capped at an application-defined number of versions. The repository contains
source/config artifacts only; run state, credentials, and provider data are not committed.
...
This is the same snapshot / diff / restore shape Spaces documents and Prompt Studio use through the
transport-agnostic `VersionHistory` primitive. Spaces keeps its SQLite `document_versions` records
for realtime conflict/retry bookkeeping and legacy fallback, while Git is the user-facing source
history; see [Spaces and RAG](/docs/core/spaces-rag) for that surface.
...
registry (`apps/core/src/runnable/workflow_builder.rs`): `workflow_builder.get_workflow`,
`workflow_builder.create_workflow`, and `workflow_builder.configure_workflow`. This is the workflow
...
typed message. A workflow whose entry never consumes the run input would silently ignore everything
you type, so it is filtered out of the mention list and rejected (with a clear error) if you target
it directly. `GET /workflows` stamps this as a `chat_input` field on each workflow.

On the wire, the chat turn carries a `workflow_id` alongside `agent_id` / `team_id`:
