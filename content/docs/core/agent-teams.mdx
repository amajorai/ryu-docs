...
For the desktop how-to (creating a team, dragging agents in, `@team` mentions), see [Teams (user guide)](/docs/surfaces/desktop/user-guide/teams). This page is the reference for the store, the strategies, and the dispatch.
...
Teams expose plain CRUD plus two member endpoints for incremental add/remove (the Teams Companion uses these for member management).
...
Team members run with auto-recall off: one user message fanned out to N members would otherwise trigger N redundant retrievals on the same query. See [Search and recall](/docs/surfaces/desktop/user-guide/search-and-recall) for where recall is wired (the single-agent chat handler).
