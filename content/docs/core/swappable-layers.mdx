...
description: How Ryu swaps the provider behind web search, browsing, computer control and memory without changing the tools an agent calls
---

A **layer** is a capability with more than one provider you can pick between: web search, page
extraction, crawling, browser control, computer control, memory. Selecting a different provider
re-points a stable set of tools at it. The agent keeps calling `web__search`; only who answers
changes.
...
`web__search`, `browser__snapshot`, `computer__click` and `memory__store` are Core's own tool ids
...
```mermaid
flowchart LR
  A["Agent calls web__search"] --> B["Capability facade"]
  B -->|"resolve capability"| C["Bound provider"]
  C --> D["exa__search"]
  C -.->|"after a swap"| E["tavily__search"]
...
| `memory` | com.ryu.memory (default), mem0, honcho | |
| `browser.control` | com.ryu.browser (default), agentbrowser | |
...
provider swap has been performed in a running app.
</Callout>
...
changes **which computer gets typed on**: `ghost` drives the machine Ryu runs on, `bytebot` drives
...
local) stays quiet and `computer.control` reads "this computer" against "remote desktop".
...
A provider that declares a capability with an **empty** `tools` map still joins resolution and can
win the pick, which would make every verb of that layer disappear with no error. The read model
reports `serves_verbs` so the picker can show such a provider as unselectable instead.
...
default, so on a fresh install there was no `web__search` tool, no row in the picker, and nothing
...
that needs no credential. Its `web__search` binding is an adapter that calls Exa's keyed REST API
