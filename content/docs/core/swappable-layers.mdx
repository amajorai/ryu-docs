...
On a node that runs the [Virtual Desktop](/docs/desktop/user-guide/virtual-desktop), Ghost's
...
- `serves_route` — the provider is served by Core calling its sidecar route directly, with no verbs
  involved. `document.parse` works entirely this way: all four parsers declare an empty `tools` map
