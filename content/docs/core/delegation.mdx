...
agent-native `delegate.fanout` tool, as the `AgentDelegate` workflow node, and as a streaming
...
fan work out mid-task by calling the built-in `delegate.fanout` tool (`apps/core/src/sidecar/mcp/delegate.rs`),
...
`delegate.fanout` takes a `tasks` array (each `{ task, agent_id? }`) plus optional `max_concurrent` /
...
Because the tool is pre-installed and a delegated sub-agent can itself hold it, a process-global semaphore
