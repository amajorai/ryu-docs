...
**Ryu Core** is the local Rust backend. It runs on `127.0.0.1:7980` by default. It decides what
runs: engine processes, conversations, long-term memory, tools, workflows, schedules, and the MCP
(Model Context Protocol) registry. Desktop, CLI, Island, Extension, and Mobile clients all use the same HTTP API. Every
model call passes through the Gateway. See [Core vs Gateway](/docs/start-here/architecture/core-vs-gateway)
...
<Cards>
  ...
  <DocCard href="/docs/standalone/passport" title="Ryu Passport" description="Encrypted, tenant-separated identity connections for agents." />
</Cards>
