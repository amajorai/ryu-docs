...
`search_conversations` and `spider`, so the `<server>.<tool>` id scheme, per-agent allowlist,
...
allowlist grants `threads.*` can spawn workers.
...
| `threads.list_threads` | List threads with status, title, message count, and pinned/archived flags. Pinned sort first. | none |
| `threads.read_thread` | Read the most recent messages plus the thread's `run_status`. | `thread_id` |
| `threads.send_message_to_thread` | Send an instruction and run the worker's agent. | `thread_id`, `message` |
