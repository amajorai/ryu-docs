...
For model-level continuation invariants, the repository's [Formal verification](/docs/extend/develop/formal-verification)
lane proves the bounded state-machine rules separately. That certificate does
not claim that a live conversation, model response, or client surface completed
the goal; those remain runtime evidence questions.
...
In Ryu Bot, the sidebar's **Goals** view gathers goal records from recently active conversations.
It separates active goals from paused/passive work and lets you return to the owning conversation
or pause and resume the existing goal record. It is a projection of the goal plugin's state, not a
second goal store.
