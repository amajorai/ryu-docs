...
```mermaid
flowchart LR
  Source["Agent, workflow, or automation"] --> Pending["Pending approval\nparked in Core"]
  Pending --> Decision{"Human decision"}
  Decision -->|approve| Action["Run or resume action"]
  Decision -->|reject| Closed["Rejected\nno side effect"]
  Action --> Recorded["Result + audit trail"]
```
...
first-person intent shown above the explanation in approval cards (for example, `Can I run
\`gmail.send_email\` now?`); the summary remains the plain-language description of what will happen.
...
the last `.`), and deliberately excludes broad read verbs (`get` / `list` / `search` / `read`) so it
...
that per-agent gate. This composes with Layer B's global `approval-mode`; either policy can require
approval. Layer C, an authoritative fail-open Gateway consult, is not yet OR'd in at the call site.
</Callout>
