---
title: "Managed data plane"
description: "How one hosted Gateway serves many organizations, resolving each request's org, budget, and policy from a per-request token."
---

The same Gateway binary that Core runs locally can be deployed once and shared by many organizations as the hosted managed-cloud **data plane**. In that topology, clients point at one Gateway and each carries its own minted `rgw_` token. The Gateway resolves that token to an org, its credit budget, and its effective policy on every request, then enforces a pre-flight credit gate before dispatch. This page explains how that multi-tenant path works.

## Managed credential lifecycle

Provisioned nodes contact the public control-plane API for bootstrap and enrollment.
When the website and API use different origins, the website origin is reserved for
browser sign-in and billing callbacks; it is not the node bootstrap endpoint.

Provisioning is a two-phase exchange. A short-lived bootstrap credential can obtain one sealed
credential generation for its assigned node. Core durably stores the returned **node-control** and
**inference-relay** credentials, verifies the node and organization binding, then acknowledges the
bootstrap. Repeated exchanges return the same pending generation rather than minting competing
pairs. Rotation follows the same persist-then-ack rule.

The data plane validates credential purpose and requested operation in addition to organization,
team, node, expiry, and revocation state. Inference routes accept the relay credential; lifecycle
and resolution routes accept the control credential. A credential with conflicting tenant
authority or an omitted operation is rejected instead of falling back to broader access.

<Callout type="warn">
This is a **hosted-only** path and runs as a **single replica**. Every counter (budgets, rate limits, the resolve cache) is an in-process map, so N replicas each grant a full quota. Shared cross-replica state is not built. Deploy one instance per fleet. When there is no `CONTROL_PLANE_URL`, the dynamic path is a no-op and the Gateway behaves as the ordinary single-org process documented in [Configuration and auth](/docs/gateway/configuration).
</Callout>

## Per-request org identity

A standalone Gateway authenticates one org at startup with its own `rgw_` key. The multi-tenant path instead resolves the org **per request** from whatever `rgw_` bearer the client sent. Any valid, non-revoked token resolves to its own org.

```
Authorization: Bearer rgw_<org-token>
```

Resolution goes through `resolve_token` (`apps/gateway/src/policy/mod.rs`), which calls the control plane:

```
GET {CONTROL_PLANE_URL}/api/control-plane/gateway/resolve
x-gateway-key: rgw_<org-token>
```

The response maps to a `ResolvedOrg` carrying the fields the pipeline needs to attribute and gate the request.

| Field | Meaning |
|---|---|
| `org_id` | The organization the token belongs to |
| `managed_inference` | Whether the org bills through the credit wallet. Only managed tenants get the pre-flight credit gate; BYOK / self-hosted do not |
| `remaining_budget_micro_usd` | Authoritative remaining credit in micro-USD, or `None` when uncapped. `Some(b)` with `b <= 0` means the wallet is exhausted |
| `policy` | The org's effective policy - `approved_models` allowlist, `locked_guardrails`, `allowed_regions`, and the cascaded firewall overlays |

The control plane has already cascaded org/team/project/user layers and honoured admin locks, so the data plane receives one resolved policy and enforces it - it never re-derives the cascade.

```mermaid
flowchart TD
  TOKEN["rgw_ organization token"] --> RESOLVE["Resolve org and policy"]
  RESOLVE --> POLICY["Approved models\nallowed regions · locked guardrails"]
  RESOLVE --> BALANCE["Remaining credit balance"]
  POLICY --> GATE["Pre-flight request gate"]
  BALANCE --> GATE
  GATE -->|"policy or balance denies"| ERROR["401 / 402\nno provider call"]
  GATE -->|"allowed"| DISPATCH["Gateway dispatch"]
  DISPATCH --> DEBIT["Post-call usage\nand credit debit"]
```

### Hard-401 on an unresolved token

An `rgw_`-shaped bearer that does not resolve (invalid, revoked, or the control plane is unreachable) maps to a hard `401`. It is never let through as anonymous, and the resolved context sets `is_master_key = false`, so admin routes still reject it.

### Policy availability and provider regions

When a Gateway is configured with a control-plane URL and node credential, the
initial effective-policy fetch is required for startup. A failed refresh keeps
the last verified policy; it does not replace it with an empty, permissive
policy. A policy-lock failure is also treated as unavailable and denies
inference.

`allowed_regions` is checked against the final routed provider before any
provider call, including fallback routes. Region metadata is explicit: local
providers use `local`, geography-neutral hosted providers use `global`, and
regional Bedrock or Vertex routes derive their region from the configured
endpoint. A provider without verified region metadata is rejected whenever a
non-empty region allowlist is active. An empty list remains the documented
unrestricted setting.

## The resolve cache

Resolving a token is a control-plane round-trip, so the Gateway caches the outcome with separate lifetimes for successful and failed resolutions. The raw token is only used as an in-memory key and is never logged.

| Outcome | TTL | Why |
|---|---|---|
| Positive (resolved org) | 60s | Keeps the hot path off the control plane, and bounds budget staleness: a topped-up org auto-recovers within one window |
| Negative (invalid / revoked / unreachable) | 10s | Blunts a resolve-DoS from a flood of bad bearers, but short enough that a just-minted token is not locked out for long |

The cache holds at most 4,096 results, removes expired results during access, and evicts the oldest result when full. Bearers longer than 256 bytes are rejected before resolution. At most 32 uncached resolutions can run at once; requests beyond that limit, or another request already resolving the same token, receive a retryable `429` response without waiting or contacting the control plane again. These temporary failures are not cached as invalid credentials. Existing fresh cache entries remain usable while resolution is busy. A rejected request does not call a model provider.

The cache is enabled only when a managed control plane is configured.

## The pre-flight credit gate

Credit enforcement is **pre-flight**, in `preflight_credit_gate` (`apps/gateway/src/pipeline/mod.rs`, run inside `enforce_budget`). It fires before dispatch, so an empty wallet blocks the request rather than one that already ran.

```mermaid
flowchart TD
  A[Request with rgw_ token] --> B{Resolves?}
  B -- no --> H[401 - never anonymous]
  B -- yes --> C{managed_inference?}
  C -- no --> P[Proceed - BYOK/self-hosted not gated]
  C -- yes --> D{remaining_budget <= 0?}
  D -- yes --> E[402 insufficient_credits]
  D -- no --> P
```

The gate reads the freshly-resolved balance (bounded by the 60s cache), so it **auto-recovers** after a top-up. It deliberately does not use a sticky wallet-empty flag, which would strand a topped-up org.

`insufficient_credits` is a distinct error from `budget_exceeded`. Both return HTTP `402`, but they mean different things.

| Error code | HTTP | Meaning |
|---|---|---|
| `insufficient_credits` | 402 | A managed org's credit wallet is exhausted (the pre-flight gate) |
| `budget_exceeded` | 402 | A per-user / per-agent / per-session charged-cost cap was hit ([Reliability](/docs/gateway/reliability)) |

The post-call credit debit (which subtracts the served call's cost from the org wallet) is documented under `[credits]` in [Configuration and auth](/docs/gateway/configuration).

## Per-tool-call debits

The model-call debit above bills the model call. On the managed plan a request can also run **Composio**
tool actions, and Composio charges per action execution, so those are billed separately. Each tool
loop returns `(value, billable_count)`; `run_tool_loop` (`apps/gateway/src/tools/mod.rs`) counts the
executed `composio.*` calls (prefix `COMPOSIO_TOOL_PREFIX`) that pass the allowlist gate. Denied,
builtin, MCP, and app tool calls are free and never counted.

At request end, `spawn_tool_call_debit` (`apps/gateway/src/pipeline/mod.rs`) fires one debit for the
whole request - `count × cost_per_tool_call_micro_usd`, at cost - against the org wallet.

| Field | Value | Why it matters |
|---|---|---|
| `reason` | `composio` | Distinguishes the tool-call ledger row from the `gateway_usage` model-call row |
| `refId` | `{request_id}:composio` | A **distinct** ref from the model-call debit's `{request_id}`. `applyDelta` is idempotent on `refId`, so reusing the request id would dedupe the tool debit against the model-call debit and silently drop it |
| Cost source | `[credits] cost_per_tool_call_micro_usd` | Per-executed-call rate; the default is `300` micro-USD ($0.30 per 1,000 standard Composio actions) |

The debit is spawned off the response path so it never adds client latency, and it is a no-op when
credits are inactive, the org is absent, the billable count is zero, or the per-call cost is zero.
Managed nodes provision the rate through the `GATEWAY_CREDITS_COST_PER_TOOL_CALL_MICRO_USD` env var
(`from_env` in `apps/gateway/src/config.rs`).

<Callout type="info">
  The service-to-service tool path (`POST /v1/exec/tool`, trusted-forwarder / master-key only) has no
  per-user org context, so it is not metered here - it is bounded by the exec budget instead, covered
  on the [tools](/docs/gateway/tools) page.
</Callout>

<Callout type="warn">
Credit debit failures are fail-closed by default. A transport error, non-success response, or
malformed success response marks accounting unavailable and the next managed request returns the
retryable `credit_accounting_unavailable` (503) error. This is kept separate from
`wallet_empty_action`, so a funded organization is not permanently classified as empty. Do not
disable `GATEWAY_CREDITS_FAIL_CLOSED` on a managed fleet; the Gateway refuses that combination at
fleet startup.
</Callout>

## Sandbox metering

An org-bound remote sandbox is metered through the same Gateway-to-control-plane debit boundary.
If that debit cannot be confirmed, the Gateway returns `kill_accounting` and Core destroys the
sandbox instead of continuing provider or compute spend without an accounting record. A malformed
or unknown tick response is also treated as an accounting failure. The final short-run tail is
retried before a completed billable execution is reported; if it still cannot be confirmed, the
execution reports an error after the remote workspace is destroyed. Unattributed local sandboxes
remain outside the billing path.

## Deploying a managed Gateway

A managed Gateway is a long-running service. Run one instance for the deployment and keep its
configuration and audit data on durable storage. On a non-loopback bind, set `GATEWAY_MASTER_KEY`
before the service starts. Keep the health endpoint public only when your deployment needs an
external liveness check; protect all model and admin routes with Gateway auth.

## Related

<Cards>
  <DocCard href="/docs/gateway/configuration" />
  <DocCard href="/docs/gateway/reliability" />
  <DocCard href="/docs/gateway/security" />
  <DocCard href="/docs/gateway/governance" />
</Cards>
