...
## Fleet provider-key custody

Hosted provider keys are owned by the control plane, not by customer nodes or
the Gateway deployment environment. This includes the ordinary providers and
the Cloudflare/Bedrock/Vertex/OpenAI-credit pool credentials. The server stores
each key sealed in the provider vault. At startup the Gateway calls:

```
GET {CONTROL_PLANE_URL}/api/key-vault/keys
x-ryu-internal-secret: <RYU_VAULT_INTERNAL_SECRET>
```

The response is applied only to the Gateway's in-memory provider registry. Set
`RYU_PROVIDER_VAULT_REQUIRED=1` on production fleet replicas so a missing or
empty vault cannot silently fall back to a raw provider env key. A successful
required-mode response is authoritative: an omitted supported key disables that
provider instead of using a stale gateway env/file key. Rotate keys through the
platform-admin vault route and restart/redeploy the Gateway to rebuild its
registry. Local standalone Gateways can omit the vault variables and continue
using their normal local provider configuration.

## Dedicated managed-channel inventory

Managed-node checkout claims one dedicated company-owned Telegram bot and one
dedicated company-owned Discord bot per node when an available allocation
exists. The control plane stores each provider token sealed in
`ManagedBotAllocation`; a customer node receives only its own channel
credential. There is no shared bot identity across customers.

Platform operators import inventory through the admin-only API:

```http
POST /api/managed-bot-inventory
Content-Type: application/json

{"provider":"discord","externalId":"<application-id>","username":"<name>","token":"<bot-token>"}
```

The route requires the platform-admin allowlist and step-up verification.
`GET /api/managed-bot-inventory` returns metadata only and never returns
tokens. Assigned identities are immutable; use
`POST /api/managed-bot-inventory/retire` for an available identity that must
never be assigned. A managed allocation is also retired when its managed node
is destroyed, so its provider identity is never recycled. For an older
deployment, import the legacy env once with
`POST /api/managed-bot-inventory/import-bootstrap`; it returns metadata and
skip reasons but never tokens. Normal provisioning never reads
`RYU_MANAGED_BOT_POOL`, so remove it after the import and keep MongoDB as the
source of truth.
