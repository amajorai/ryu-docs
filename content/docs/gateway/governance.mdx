...
2. **Owner-scoped self-grant.** The scope's namespace (the token before its first `:` or `.`)
   ...
   `com.ryu.monitors` may declare `monitors:crud`, and `com.acme.invoices` may declare
...
`com.evil.monitors` derives the same owner namespace as `com.ryu.monitors`. Reserved namespaces
