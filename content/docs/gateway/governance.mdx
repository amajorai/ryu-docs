...
The fence has to cover **every** namespace Core gates on `approved_grants`, not just the ones an
allowlist entry happens to mention. `runtime:external` (the venv + `pip install` provisioning gate
...
`com.evil.monitors` derives the same owner namespace as `@ryu/monitors`. Reserved namespaces
keep such a squat out of the host primitives, but the app-owned families it *can* reach do front
real Core endpoints (`monitors:crud` fronts `/api/monitors/*`). The containment that matters is
downstream: those routes are gated on the **owning app being installed and enabled**, so a squatter
holding the grant reaches nothing the user has not already turned on. Publisher-owned id namespaces
at the registry are the real fix.
</Callout>
...
the package kind and version to the package digest, archive digest, and entitlement-gated download
