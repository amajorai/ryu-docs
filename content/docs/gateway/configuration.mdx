...
`[composio]` wires in the direct Composio action-execution backend. This is separate from the
OAuth-backed **Composio Connect** remote MCP plugin, which does not use `api_key` or
`COMPOSIO_API_KEY`.

The direct backend is disabled unless `api_key` is set (or `COMPOSIO_API_KEY` is in the environment).
