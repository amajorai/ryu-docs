...
| `[computer_use]` | Node-wide Ghost computer-use permissions, including locked use | this page |
...
The `[computer_use]` section is the node-wide policy for Ghost, Ryu's computer-use
provider. In the desktop, it appears as **Computer settings** for the selected computer
...
computer actions remain foreground-only.
...
| Field | Default | Meaning |
|---|---|---|
| `cost_per_tool_call_micro_usd` | `299` | Per Composio tool execution, i.e. `$0.299` per 1,000. Billed at cost |
...
| `RYU_ANALYTICS_INGEST_URL` | Private Ryu-owned typed analytics relay URL. Hosted deployments set this; Core-managed Gateways receive it only with a node-bound relay key. |
| `RYU_ANALYTICS_ENABLED` | Core-managed local-Gateway gate for Ryu's content-free product analytics. It is separate from `RYU_DIAGNOSTICS_EXPORT_ENABLED`. |
