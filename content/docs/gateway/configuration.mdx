...
response that carries no timing at all. TTS and STT have no metered path, so their flat rate is the
...
<Callout type="warn">
  ...
  explicit `0` on the tool-call, TTS or STT rate is a **startup failure** — those three have no
  second cost source. To give a modality away deliberately, set
