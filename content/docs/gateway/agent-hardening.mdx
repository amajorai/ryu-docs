...
```mermaid
flowchart TD
  PROFILE["Selected profile\nx-ryu-tool-profile"] --> ALLOW["Profile allow set\nor wildcard for full"]
  HEADER["Explicit request ids\nx-ryu-tools"] --> ALLOW
  ALLOW --> DENY["Remove profile.deny\ndeny wins for scoped profiles"]
  DENY --> ALWAYS["Append always_on\nnever deny-stripped"]
  ALWAYS --> WILDCARD{"Wildcard *?"}
  WILDCARD -->|"yes"| ANY["Any catalog tool may pass"]
  WILDCARD -->|"no"| EXACT{"Exact FQ tool id allowed?"}
  EXACT -->|"yes"| RUN["Run through governed exec path"]
  EXACT -->|"no"| REJECT["Deny and emit an error result"]
  ANY --> AUDIT["Exec budget + audit"]
  RUN --> AUDIT
```
...
Static drift warnings are useful for contradictions, but an operator also needs to know when a
node is exposed, missing a provider, has an incomplete firewall, or has unbounded local work. The
read-only Doctor report combines those checks with the Core-side checks that Gateway cannot see:
...
does not include credentials. The desktop renders the same report under Gateway → Diagnostics and
on the onboarding safety step.

For an explicit repair preview, call `POST /api/gateway/doctor/fix` with `{ "dryRun": true }` or
use `ryu doctor --dry-run`. Applying requires the separate `--fix`/**Apply safe fixes** action. The
repair allowlist covers only protective Gateway flags; decisions involving exposure, credentials,
agent routing, approvals, providers, budgets, or concurrency are reported for a human to choose.
...
| `RYU_AGENT_EGRESS_ALLOW_HOSTS` | empty | Comma-separated, case-insensitive allowlist that bypasses the screen. Entries may be a bare host (`example.internal`) or a host and port (`127.0.0.1:3000`) |
