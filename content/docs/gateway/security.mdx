...
The firewall is the Gateway's data-loss-prevention layer. It
...
<Callout type="warn">
  ...
  and is unioned onto the built-in sets through the policy cascade below. Manage custom patterns through Gateway settings, `gateway.toml`, or the authenticated
  configuration API.
</Callout>
...
The Gateway's firewall is not one flat process-global config. It resolves as a three-level cascade per request , so a box admin, an organization, and an individual agent can each set policy.
...
Alongside the regex scanner, the firewall can run an opt-in cheap-LLM inspector that asks a model whether the inbound turn is a prompt injection or leaks PII / secrets, then applies the same `FirewallPolicy` action (Block / Sanitize / Warn) to a flagged turn.
...
subscription-preserving passthrough proxy  adds DLP on the
