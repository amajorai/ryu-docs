...
Interactive requests are served ahead of background fan-out (delegate calls, thread workers, scheduler). The gate is in the Gateway because it governs a shared resource; Core cannot see other callers' in-flight requests. Streaming output guardrails keep the permit while the local generation is being buffered, then release it before any re-entrant LLM judge runs; deterministic output evaluators never bypass admission.
