...
When the model calls `tool_search`, the gateway queries Core's catalog, describes the top hits, and injects their full tool definitions into the request so the model can call them on the next round. The result the model receives is a compact descriptor list (`id`, `name`, `description`).
