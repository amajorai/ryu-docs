...
This surface is the unified tool gateway. The Core-side catalog it queries, and the
programmatic-tool-calling sandbox it forwards to, are documented elsewhere:
...
When the model calls `tool_search`, Core's unified catalog uses the default Needle 2 selector to
...
result the model receives is a compact descriptor list (`id`, `name`, `description`). Needle 2 is
...
The Core catalog includes two read-only tools for quota-aware agents:
...
read-only meter rows. An unavailable or unsupported provider is returned as
`available: false` with a structured `reason`; it is never represented as a guessed zero.

The Gateway still gates both tools by the normal fully-qualified allowlist and audits the call.
...
Matching is exact on the FQ id. Bare-name and bare-server entries are deliberately rejected: an entry like `search` must not authorize `exa.search` or `composio.search` across planes. To grant a whole server, use the explicit `<server>.*` id form upstream, never a bare-server equality.
...
exposes one governed endpoint (`apps/gateway/src/tools/exec.rs`). PTC uses the same Needle 2-backed
