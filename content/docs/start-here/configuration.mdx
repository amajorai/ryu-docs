...
Telegram, Slack, Discord, WhatsApp and iMessage bots are configured **per bot**, not per
platform. Each bot is its own record with its own token, its own agent, and its own reply
policy.
...
| `channelType` | `telegram`, `slack`, `discord`, `whatsapp`, `bluebubbles`. |
