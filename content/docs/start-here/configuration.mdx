...
Core also accepts a structured, non-secret JSON file at the active OS config path:
`ryu/node-config.json`. A copyable starting point lives in
[`config/node-config.example.json`](https://github.com/amajorai/ryu/blob/main/config/node-config.example.json).

The file uses grouped settings and converts them to the existing compatibility
names at boot, which lets the existing readers migrate without a flag-day rewrite:
...
An `overrides` object is also accepted for a non-secret `RYU_*` name that has not
yet received a friendly path. It is a compatibility escape hatch, not a place for
credentials.

The precedence is **explicit environment -> Settings preference -> node config ->
built-in default**. This makes the file safe to adopt incrementally: an existing
deployment can keep its environment values, while a local node moves ordinary
model, RAG, sandbox, voice, and feature defaults into one editable document.
