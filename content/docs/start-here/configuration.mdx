...
The [public environment variables](/docs/start-here/environment-variables) page lists the
variables you may need for a self-hosted node, the Gateway, or a client. This page is the other
half: what the primary config surfaces are, and how precedence resolves between them.
...
Ryu chooses a file format by ownership rather than forcing every surface into one syntax:
...
tool keep their native format. The same logical setting should have one source of truth; a JSON or
TOML projection is not a second configuration store.
