...
Beta only if you are testing the current picker behavior. Choose Nightly for a snapshot every 3 hours
when `main` has moved, and Canary when you need the newest published build. Use Dev when you are
developing Ryu itself or need a separate local stack for debugging.
