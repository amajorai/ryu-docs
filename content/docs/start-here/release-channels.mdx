...
| --- | --- | --- | --- | --- |
| **Stable** (shown as **Research Preview** in the desktop brand) | `1.2.3` | The recommended signed release train | Stable release feed | Lowest change rate and the default choice |
...
stable desktop can be pointed at the Canary feed from **Settings -> Updates**. The build badge
identifies the installed build, while the release-channel setting identifies the train it checks.

## Local data and state

The default profile mapping is the part that answers whether two channels share local Core data.
The ports below are the Core and Gateway ports; all profile-aware sidecars use the same offset. The
[Port Map](/docs/reference/defaults/ports) describes the rest of the loopback services.
