...
| Kind | What it is |
|---|---|
| `agent` | A multi-turn model loop with configurable model, tools, memory, and persona settings |
...
they need, packaged so Core can install, enable, disable, and update it as a unit. It is a declarative
file describing everything the plugin contributes.

An agent inside a plugin carries its own model, tools, memory, persona, and policy configuration:
...
Each agent can be configured for a different job, and you can change its settings without rebuilding
the agent.
