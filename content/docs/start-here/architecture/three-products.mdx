...
- Local Voice Recognition (whisper.cpp) and Audio (OuteTTS)
...
- Budgets (per-user, per-agent, per-session charged-cost caps)
...
The products run the same code. They differ in what runs where, not in what
the code does.
...
- The Gateway is AGPL, preventing proprietary forks from reselling it
