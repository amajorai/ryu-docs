...
power-user and admin workspace. **Ryu Gateway**, **Ryu Box**, **Ryu Mail**, **Ryu Notify**, and
**Ryu Hire** are standalone services that can also plug into the workspace. **Ryu Compute** and
**Ryu Share** are the early-access exchange surfaces for renting governed capacity and offering
idle nodes. **Ryu Platform** is the developer layer. **Ryu Apps**
...
These names describe different boundaries. They are not competing runtimes or
different copies of the same interface.
...
flowchart TB
  ...
  MAIL["Ryu Mail\nAgent inbox API"] --> Platform
  NOTIFY["Ryu Notify\nEvent inbox + SSE"] --> Platform
  HIRE["Ryu Hire\nPay-per-run specialists"] --> Platform
  COMPUTE["Ryu Compute\nRent governed lanes"] --> Platform
  SHARE["Ryu Share\nOffer idle nodes"] --> Platform
  Platform --> Infra
```
...
| **Ryu Mail** | Agent builders and operators | Give agents inboxes they can receive from and send through an authenticated API. |
| **Ryu Notify** | Product teams and operators | Store product events and consume them through bounded REST history or SSE. |
| **Ryu Hire** | People with a one-off task | Recruit a managed specialist for one run and pay with credits, without installing it. |
| **Ryu Compute** | People who need temporary capacity | Choose a bounded CPU or GPU workcell; the early-access Desktop surface does not submit live leases yet. |
| **Ryu Share** | People with idle nodes or agents | Configure when a node may offer a workcell or approved agent; publishing and payouts are not live yet. |
...
[Ryu Box](https://ryuhq.com/products/box), [Ryu Mail](https://ryuhq.com/products/mail),
[Ryu Notify](https://ryuhq.com/products/notify), [Ryu Hire](https://ryuhq.com/products/hire),
[Ryu Compute](https://ryuhq.com/products/compute), [Ryu Share](https://ryuhq.com/products/share),
and [Ryu Platform](https://ryuhq.com/platform).

The exchange surfaces are currently an early-access Desktop experience; see
[Compute and Share](/docs/surfaces/desktop/user-guide/compute-share) for their product split and
the current lease and provider boundaries.

The standalone service domains are API entry points. Their browser home routes
lead back to the signed-in organization dashboard, where the organization
**Products** view manages Box workspaces and Agent Inboxes alongside the existing
Gateway controls.
...
The [public product catalog](https://ryuhq.com/products) keeps these boundaries visible: each
product page uses the same Ryu content and surface language while arranging its story around the
job that product owns.
...
- **Ryu Mail** provides Agent Inboxes through the Mail service. The self-host process owns its
  mailbox state, SMTP hand-off, and per-inbox HMAC inbound route.
- **Ryu Notify** provides a separate event inbox for products and agents. It is not Core's
  user-scoped notification/HITL feed; its first service contract is HTTP history plus SSE.
...
| Give an agent an API-backed email inbox | [Ryu Mail](https://ryuhq.com/products/mail) |
| Record product events and follow them live | [Ryu Notify](https://ryuhq.com/products/notify) |
