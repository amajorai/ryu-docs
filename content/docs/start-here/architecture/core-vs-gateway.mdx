...
The Gateway is the control layer. Agents are commoditized; the control layer around them is not. Keeping
...
provider-native protocols that expose no redirect hook remain genuine chat-egress bypasses.

The default OpenAI-compatible route and new/auto-installed routable ACP agents route through the
Gateway by default. Gemini CLI, OpenClaw, Hermes, and any other non-wirable agent remain bypasses.
When you need firewall, budget, and audit coverage to be absolute, use a gateway-routed agent and
leave its direct-egress opt-out disabled.
...
<Callout type="info">
  The Gateway is already on the default chat path for routable agents. The remaining boundary is
  the provider-native ACP bypass (plus governance **surfaces** such as firewall, PII/DLP, and budget
  configuration UIs), not the data-plane wiring.
</Callout>
...
Continue with [Architecture](/docs/start-here/architecture), [Core](/docs/core), [Gateway](/docs/gateway), [Integrate with Ryu](/docs/extend/integrate), and [Gateway routing](/docs/gateway/routing).
