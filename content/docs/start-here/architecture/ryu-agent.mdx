...
4. **Exposes Core's tools to Pi** — the managed extension lists and calls tools through Core. Core
   ...
   flagship is seeded with the explicit `*` grant; newly-created agents start with no executable
...
| **Gateway routing** | **On by default** for the routable ACP entry; direct-egress opt-out | **On by default** (`gateway_bypass: false`) |
| **Model calls** | Gateway-governed by default (unless opted out) | Always through Gateway |
...
5. Pi runs its full tool loop (model → tool calls → tool results → continue) via ACP protocol.
...
rechecks ownership and current agent policy on every call. The capability expires, is revoked when
the session is replaced or closed, and is invalidated when the node token rotates. It cannot
authenticate to other Core routes.

## The Gateway integration

The Ryu agent is the first-run flagship, and its Gateway routing is always on. Other routable ACP
agents use the same governed-by-default foundation through their applicable mechanism. Two
mechanisms ensure Ryu's model calls traverse the Gateway:
...
| `auth.json` | The *active* account's API key / OAuth subscription tokens per provider. OAuth tokens are proactively refreshed before each turn. |

Credentials are sealed with the at-rest master key (OS keychain → `~/.ryu/master.key`) and stored in
the multi-account vault at `~/.ryu/pi-accounts.db`; `auth.json` only ever materializes the active
account, so Pi keeps reading the file format it always did.
...
| Audio engine | OuteTTS / Kokoro | Yes |
| Image model | stable-diffusion.cpp | Yes |
| Tools / MCP | Core's allowlisted registry | Yes — via MCP registry and plugin system |
