...
**routable** model call to the Gateway for governance. The flow is the same on every request that
the selected engine exposes to Ryu:

```mermaid
flowchart TD
  S["🖥️ Desktop · 💬 Bots · ⚡ Gateway API"] --> C
    C["Ryu Core · orchestration\nsessions · memory · tools · workflows · sub-agents · sidecars\ndecides WHAT RUNS, then routes routable model calls to the Gateway"]
    C -->|"routable model call"| G
...
paid for* it is Gateway. That split has its own page. Provider-native runtimes that do not expose a
redirect hook remain a real boundary: Ryu labels their model traffic as a bypass instead of
claiming coverage it cannot enforce. Use the [Core versus Gateway](/docs/start-here/architecture/core-vs-gateway)
page for the current coverage matrix.
...
flowchart LR
  ...
  Core -->|"routable model call"| Gateway
```
...
The infrastructure becomes useful when it feels like delegation. Ryu is moving toward an
outcome-first Bot experience inspired by the best parts of [Manus](https://manus.im/) and
[Grok Bot](https://x.ai/news/introducing-grok-bot): give Ryu an outcome, see a short plan, let it
work while you leave, approve only consequential actions, and return to a receipt containing what
actually happened.

That experience sits on the durable contracts Ryu already exposes for runs, workflows, approvals,
artifacts, routines, and agent teams. The [last-mover agent](/docs/start-here/architecture/last-mover-agent)
page describes the product contract and the boundaries we will not hide.
...
| **Outcome-first** | A long or consequential request becomes a durable task with a plan, progress, evidence, and a truthful outcome. |
