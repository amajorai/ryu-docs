...
tags: ["Automation"]
catalog: {"kind":"plugin","id":"@ryu/agent-comms","category":"Automation","version":"0.1.15","official":true,"builtIn":false,"system":false,"preInstalled":false,"stability":"experimental","hidden":false,"surfaces":{"gateway":"none","core":"full","desktop":"full","island":"limited","mobile":"limited","extension":"limited","web":"full","cli":"full"}}
---
...
### `agents.react` - React to a Message

Set an emoji reaction on an exact persisted message in the calling agent's current conversation. Use the message_id from the conversation or from search_conversations.search; the target may be a user message or a message authored by any other agent in the same conversation. Repeating the same reaction is idempotent. The actor and conversation are resolved by Core from the calling agent and cannot be supplied by the model.

Backend: inline_deno

| Parameter | Type | Required | Description |
|---|---|---|---|
| `message_id` | string | yes | The exact persisted Core message id to react to. Streaming/client-generated ids and ids from another conversation are rejected; messages authored by other agents are allowed. |
| `emoji` | string | yes | The emoji or bounded emoji sequence to add, such as ✅ or 👀. |

## Permissions

**Grants:** `tool:execute`, `storage:kv`, `conversation:reactions`, `tool:http-egress:127.0.0.1`
