...
tags: ["Media & Voice"]
catalog: {"kind":"app","id":"@ryu/clips","category":"Media & Voice","version":"0.1.14","official":true,"builtIn":true,"system":false,"preInstalled":false,"stability":"experimental","hidden":false,"surfaces":{"gateway":"none","core":"full","desktop":"full","island":"none","mobile":"none","extension":"none","web":"none","cli":"none"}}
---

## What it does

A local-first video capture and ingest surface with a bundled editor preview. The out-of-process ryu-clips sidecar owns Core→Shadow recording and URL ingest; transcript-led analysis and vertical rendering are not connected yet. The Companion's toolbar labels bundled media as sample data even when a host is connected; a source URL must be ingested before node media is available. Source export reports a browser download request and cannot confirm that the browser saved the file.
...
- **Clips widget** - No chat widget template is shipped.
