...
On desktop startup, the node step can also choose which computer or cloud workspace to connect to
before the shell opens. Set a startup node in **Settings → General → On startup** to reuse it with
**Use defaults**, choose **Always ask** to review it every launch, or choose **Ask at startup** for
the node default when you want to be prompted again. This startup choice sets the window's default;
individual tabs can still use the node selector to target a different node.
