...
<Quiz
  questions={[
    ...
    {
      q: "How are Agent, Workflow, Tool, Skill, and MCP-server related in the type system?",
      options: [
        "They are peer Runnable kinds, not a hierarchy",
        "Every kind inherits from Agent",
        "Workflow owns every other kind",
      ],
      answer: 0,
      explain:
        "The authored kinds are peers with the same input-run-output contract, which lets them compose in both directions.",
    },
    {
      q: "Which kinds exist internally but are not among the five developer-authored kinds in this lesson?",
      options: [
      ...
      answer: 0,
      explain:
        "Companion, Channel, Engine, and Policy are additional internal Runnable kinds. Developers directly author Agent, Workflow, Tool, Skill, and MCP-server.",
    },
  ]}
/>
