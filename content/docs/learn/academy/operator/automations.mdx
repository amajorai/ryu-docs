...
Open **Calendar → Agenda** to review the list view. Each populated day includes a 24-hour status
strip: green for completed runs, red for failures, and amber for projected scheduled runs. The
strip is a compact time map, while the rows below it keep the schedule name and details readable.
