---
title: Agent setup
description: Configure an agent's instructions, model, tools, memory, voice, and safety settings.
level: 200
---

A Ryu agent is a saved assistant. Its setup controls the instructions it follows, the model and
provider it uses, the tools and apps it can call, the memory and Spaces it can access, its voice and
persona, and its safety profile. These settings are independent, so you can tailor each agent to a
job without creating a new runtime.
...
flowchart TB
  Setup([Agent setup])
  Setup --> I[Instructions + persona]
  Setup --> M[Chat model]
  Setup --> V[Voice + image models]
  Setup --> T[Tools + MCP servers]
  Setup --> Mem[Memory + Spaces]
  Setup --> Pol[Safety profile + Gateway policy]
```

Because these settings are independent, you can give one agent a small local model with no tools
for quick drafting, and another a large model wired to a research toolset and a private Space. Same
app, different configurations.

## Lifecycle is separate from capability

An agent's configuration also has an execution posture. New custom and imported agents start in
**Trial** with effective **Read-only** access. Use Trial for a manual foreground chat while you
check the prompt: read and real preview operations can run, but writes, unknown tools, external
sends, memory writes, delegation, channels, and automation are blocked. **Draft** is authoring-only
and cannot be selected or run. **Active** is the state required for workflows, schedules, channels,
and delegated work.

Active agents can save one of three safety profiles: **Read-only**, **Approval required**, or
**Autonomous**. The lifecycle and safety profile are independent, so changing a profile does not
skip the required Draft → Trial → Active review checkpoint. Demoting an agent pauses its owned
...
When you create an agent, its Tools & skills setting starts with every MCP, built-in, and enabled
plugin/app tool currently registered on the node, plus every globally enabled skill. New agents keep
that all-access default as tools and skills are added later.

Use **Disable all** or switch off individual rows when an agent should have less reach. Those choices
...
the same defaults: their saved configuration starts broad, and any routable model traffic starts
...
Only the default **Ryu** agent is installed out of the box. Everything else is opt-in through the
agents catalog.

During onboarding, Ryu detects which agent CLIs you already have on your PATH and offers to add the
matching agents. Supported ones include Claude Code, Codex, Gemini CLI, bare Pi, OpenClaw, Hermes,
and more. You stay in control of what gets added.
...
The default **Ryu** agent is a locked built-in, so you cannot change all of its settings freely. You
can still change the part that matters most - its model and provider - through a dedicated config
panel (the Ryu Pi config) that appears only when you edit the Ryu agent. Other agents are edited
through the normal agent editor.

## Talking to several at once

Council, or team, chat lets you bring more than one agent into a single conversation. Use an
@mention to address a specific agent, and you can pull several agents into the same thread to
compare answers or split a task.
  ...
  questions={[
    ...
    },
    {
      ...
      options: [
        ...
        "Makes the agent available with default settings and currently enabled capabilities",
        "Auto-configures the best model and tools",
      ],
      answer: 1,
      explain:
        "Adding a catalog agent makes it available with default settings and the normal all-enabled capability starting point; review its model, tools, and memory first.",
    },
    {
