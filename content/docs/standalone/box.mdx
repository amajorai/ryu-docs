...
The container image stores this database under the persistent `/data` volume.
Its entrypoint repairs the mounted volume ownership before dropping to the
unprivileged Box user, so redeploys keep the lifecycle store writable without
