...
Admission is tenant- and service-scoped. By default, a tenant may have up to
eight active Boxes and 16 resource units (`small` = 1, `default` = 2,
`large` = 4); the service allows up to 64 active Boxes and 128 resource units.
...
Common error codes include `unauthorized`, `invalid_request`, `quota_exceeded`,
`box_not_ready`, `box_busy`, `driver_unavailable`, `idempotency_conflict`, and
`not_found`.
