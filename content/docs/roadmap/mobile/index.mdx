...
<Callout type="info">
  ...
  pending, and several screens still need `bunx expo install` plus a device build to exercise.
  On-device inference is not wired.
</Callout>
...
## The shared client: packages/core-client

The shared client layer lives in the workspace package **`packages/core-client`** — the same client
the desktop app and the CLI/TUI run on (see
[`@ryuhq/core-client`](/docs/develop/sdk/core-client)): a
